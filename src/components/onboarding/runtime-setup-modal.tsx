  ...
  return (
    <div className="p-6">
      ...
      <div className="p-4 rounded-lg border border-border/30 bg-secondary/20 text-sm text-muted-foreground space-y-2">
        <p>Opzava reads OpenCode runtime status from the installed <code>opencode</code> CLI and scans session history from <code>~/.local/share/opencode/*.db</code>.</p>
        <p>Restart OpenCode or create a new OpenCode session if you want Opzava to pick up fresh session activity immediately.</p>
      </div>
...
function HermesSetup({ onClose, onComplete }: { onClose: () => void; onComplete: () => void }) {
  ...
  const [selectedModel, setSelectedModel] = useState(DISPATCH_MODEL_DEFAULT)
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">Connect Hermes agent to Opzava</p>
        </div>
        <button type="button" onClick={onClose} className="text-muted-foreground hover:text-foreground">
          <svg className="w-5 h-5" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"><path d="M4 4l8 8M12 4l-8 8" /></svg>
        </button>
          ...
          <div className="p-4 rounded-lg border border-border/30 bg-secondary/20 space-y-3">
            ...
            <div className="text-xs text-muted-foreground/60 space-y-1">
              ...
              <ul className="list-disc list-inside pl-2 space-y-0.5">
                ...
                <li>Enable task dispatching from Opzava</li>
              </ul>
            </div>
          </div>
      ...
      {step === 'provider' && (
        <div className="space-y-4">
          ...
          {(() => {
            const PROVIDERS = [
              { id: 'anthropic', label: 'Anthropic', hint: 'Claude', env: 'ANTHROPIC_API_KEY', hermesProvider: 'anthropic', models: [DISPATCH_MODEL_DEFAULT, DISPATCH_MODEL_COMPLEX, MODEL_CLAUDE_HAIKU_4_5, MODEL_CLAUDE_SONNET_4_5] },
              ...
              { id: 'openrouter', label: 'OpenRouter', hint: '200+ models', env: 'OPENROUTER_API_KEY', hermesProvider: 'openrouter', models: [MODEL_ANTHROPIC_SONNET_4_6, 'openai/gpt-4.1', 'google/gemini-2.5-pro', 'meta-llama/llama-4-maverick', 'deepseek/deepseek-r1'] },
