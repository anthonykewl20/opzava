...
const categoryLabels: Record<string, { label: string; icon: string; description: string }> = {
  general: { label: 'General', icon: '⚙', description: 'Core Opzava settings' },
  ...
  'Provider Connections': { label: 'Provider Connections', icon: '↔', description: 'WordPress and Resend connection settings' },
...
const categoryOrder = ['general', 'security', 'profiles', 'Provider Connections', 'retention', 'chat', 'gateway', 'custom']
...
export function SettingsPanel() {
  ...
  const hasChanges = Object.keys(edits).some(key => {
    ...
    return setting.sensitive ? edits[key] !== '' : edits[key] !== setting.value
  })
  ...
  const handleSave = async () => {
    ...
    for (const [key, value] of Object.entries(edits)) {
      const setting = settings.find(s => s.key === key)
      if (setting?.sensitive ? value !== '' : setting && value !== setting.value) {
        changes[key] = value
      }
    }
    ...
    <div className="p-4 md:p-6 max-w-4xl mx-auto space-y-6">
      ...
      {currentUser?.role === 'admin' && (
        <div className="space-y-3">
          ...
          {/* Provider Connections (WordPress + Resend) */}
      ...
      <div className="flex gap-1 border-b border-border pb-px">
        {categories.map(cat => {
          ...
          const changedCount = (grouped[cat] || []).filter(s => (
            edits[s.key] !== undefined && (s.sensitive ? edits[s.key] !== '' : edits[s.key] !== s.value)
          )).length
          return (
            <Button
              key={cat}
      ...
      <div className="space-y-3">
        {activeCategory !== 'security' && (grouped[activeCategory] || []).map(setting => {
          ...
          const inputValue = setting.sensitive ? (edits[setting.key] ?? '') : currentValue
          const isChanged = edits[setting.key] !== undefined && (setting.sensitive ? edits[setting.key] !== '' : edits[setting.key] !== setting.value)
          const isBooleanish = !setting.sensitive && (setting.value === 'true' || setting.value === 'false')
          const isNumeric = !setting.sensitive && /^\d+$/.test(setting.value)
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {setting.sensitive && (
                      <span className={`text-2xs px-1.5 py-0.5 rounded ${
                        setting.configured ? 'bg-green-500/15 text-green-400' : 'bg-muted text-muted-foreground'
                      }`}>
                  ...
                  <div className="flex items-center gap-2">
                    {setting.sensitive ? (
                      <input
                        type="password"
                        value={inputValue}
                        placeholder={setting.configured ? 'Leave blank to keep existing secret' : 'Enter secret'}
        ...
        <div className="fixed bottom-4 left-1/2 -translate-x-1/2 bg-card border border-border rounded-lg shadow-lg px-4 py-2.5 flex items-center gap-3 z-40">
          ...
          <span className="text-xs text-foreground">
            {Object.keys(edits).filter(k => {
              ...
              return s && (s.sensitive ? edits[k] !== '' : edits[k] !== s.value)
