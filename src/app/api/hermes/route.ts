...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (action === 'install-hook') {
      ...
      logger.info('Installed Opzava hook for Hermes Agent')
    ...
    if (action === 'uninstall-hook') {
      ...
      logger.info('Uninstalled Opzava hook for Hermes Agent')
...
description: Reports agent telemetry to Opzava
version: "1.0"
events:
  - agent:start
  - agent:end
