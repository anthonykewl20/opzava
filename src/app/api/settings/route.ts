...
import { resendConnectionConfigSchema, RESEND_CONNECTION_SCHEMA_VERSION } from '@/opzava/platform/admin-config/resend-connection'
import { wordpressConnectionConfigSchema, WORDPRESS_CONNECTION_SCHEMA_VERSION } from '@/opzava/platform/admin-config/wordpress-connection'
...
const providerCredentialRef = createSecretReference({
  id: 'settings-validation',
  scope: 'provider-credential',
  purpose: 'settings shape validation',
})

// Default settings definitions (category, description, default value).
// `sensitive` marks secret values (API keys / app passwords): never returned by GET,
// and an empty value on PUT does not overwrite an already-stored secret.
const settingDefinitions: Record<string, { category: string; description: string; default: string; sensitive?: boolean }> = {
  // Retention
  ...
  'chat.coordinator_target_agent': {
    category: 'chat',
    description: 'Optional coordinator routing target (agent name or openclawId); when set, coordinator inbox messages route here before the default fallback.',
    default: '',
  },

  // General
  'general.site_name': { category: 'general', description: 'Opzava display name', default: 'Opzava' },
...
const settingValueValidators: Record<string, (value: string) => boolean> = {
  wordpress_site_url: (v) => v === '' || wordpressConnectionConfigSchema.safeParse({
    schemaVersion: WORDPRESS_CONNECTION_SCHEMA_VERSION,
    siteUrl: v,
    defaultStatus: 'draft',
    credentialRef: providerCredentialRef,
  }).success,
  resend_from_address: (v) => v === '' || resendConnectionConfigSchema.safeParse({
    schemaVersion: RESEND_CONNECTION_SCHEMA_VERSION,
    defaultFromAddress: v,
    apiKeyRef: providerCredentialRef,
  }).success,
}
...
export async function GET(request: NextRequest) {
  ...
  for (const [key, def] of Object.entries(settingDefinitions)) {
    ...
    const rawValue = row?.value ?? def.default
    // Sensitive values are never returned in cleartext — only whether one is configured.
    const configured = rawValue.length > 0
    const value = def.sensitive && configured ? REDACTED_SETTING_VALUE : rawValue
    settings.push({
      key,
      value,
      ...
      is_default: !row,
      ...(def.sensitive ? { sensitive: true, configured } : {}),
    })
  }
...
async function handlePut(request: NextRequest) {
  ...
  // Validate connection values (URL / email) before persisting.
  for (const [key, value] of Object.entries(body.settings)) {
    const validate = settingValueValidators[key]
  ...
  const txn = db.transaction(() => {
    for (const [key, value] of Object.entries(body.settings)) {
      ...
      // Get old value for audit / secret-preservation
      const existing = db.prepare('SELECT value FROM settings WHERE key = ?').get(key) as { value: string } | undefined

      // A blank submit for a secret must not wipe an already-stored secret (write-only field).
      if (def?.sensitive && strValue === '') continue

      // Audit a secret change without recording either secret value.
      changes[key] = def?.sensitive
        ? { old: existing ? '[secret]' : null, new: '[secret]' }
        : { old: existing?.value ?? null, new: strValue }
