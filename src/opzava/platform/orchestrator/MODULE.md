...
- `runDecomposition(input, deps) → RunDecompositionOutcome` (`run-decomposition.ts`) — `propose → gate → persist`, surfacing the two distinct rejection layers separately (`proposal-rejected` vs `gate-rejected`). Idempotent end-to-end.
- `decomposeAndExecute(input, deps) → DecomposeAndExecuteOutcome` (`decompose-and-execute.ts`) — the **complete fleet flow**: `runDecomposition → hydrate → execute` (the workflow-engine over the persisted graph). Returns `executed{status}` | `proposal-rejected` | `gate-rejected`.
- `hydrateGraph(graph, {card, workerModel})` — the deterministic bridge (seed of assign-to-worker / M3): fills the proposed structure's step `data` from the card + edges (dispatch → `task`+`plan`, review → `title`+upstream `sourceStep`). No LLM — pure routing.
