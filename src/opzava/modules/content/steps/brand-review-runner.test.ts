...
function baseJob(overrides: Partial<Job> = {}): Job {
  return parseJob({
    ...
    stepRunId: 'step_run_br_001',
    status: 'queued',
    idempotencyKey: 'workflow:run_001:step:brand-review:v1',
...
function draftPayload() {
  ...
  let d = 0
  const draft = createArticleDraftStepService({ provider: createMockArticleDraftProvider(), newId: () => `d_${++d}`, now }).run({
    idea,
  ...
  return { articleDraftArtifact: draft.record, sourceStepRunId: 'step_run_br_001' }
}
...
describe('brand-review-runner', () => {
  it('runs brand-review through the durable runner and captures the review', async () => {
    const repo = repository()
    let captured: { record: { artifactType: string }; output: unknown } | null = null
    const job = baseJob({ payload: draftPayload() })
    ...
    const worker = createRunnerWorker({
      ...
      executor: createContentStepExecutor({
        service: service(),
        parseInput: parseBrandReviewStepInput,
        onResult: (r) => {
    ...
    expect(captured!.record.artifactType).toBe('brand-review')
    expect(captured!.output).toEqual({ kind: 'artifact', artifactType: 'brand-review' })
  ...
  it('fails the job when the upstream artifact is malformed', async () => {
    const repo = repository()
    const job = baseJob({
      jobId: 'job_br_bad_001',
      payload: {
        articleDraftArtifact: { not: 'an artifact' },
        sourceStepRunId: 'step_run_br_001'
      }
    })
    repo.saveJob(jobRecord(job))
