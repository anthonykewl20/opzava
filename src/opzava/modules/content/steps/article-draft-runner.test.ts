...
describe('article-draft-runner', () => {
  it('runs article-draft through the durable runner and captures the artifact', async () => {
    ...
    const worker = createRunnerWorker({
      ...
      executor: createContentStepExecutor({
        service: service(),
        parseInput: parseArticleDraftStepInput,
        onResult: (r) => {
    ...
    expect(captured!.record.artifactType).toBe('article-draft')
    expect(captured!.output).toEqual({ kind: 'artifact', artifactType: 'article-draft' })
  ...
  it('fails the job when an upstream artifact is malformed', async () => {
    const repo = repository()
    const job = baseJob({
      jobId: 'job_draft_bad_001',
      payload: {
        idea: VALID_IDEA,
        outlineArtifact: { not: 'an artifact' },
