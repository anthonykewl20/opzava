...
function baseJob(overrides: Partial<Job> = {}): Job {
  return parseJob({
    ...
    stepRunId: 'step_run_appr_001',
    status: 'queued',
    idempotencyKey: 'workflow:run_001:step:human-approval:v1',
...
function draftPayload() {
  ...
  return {
    ...
    sourceStepRunId: 'step_run_appr_001',
  }
}
...
describe('human-approval-runner', () => {
  it('runs human-approval through the durable runner and captures the approval', async () => {
    ...
    const worker = createRunnerWorker({
      ...
      executor: createContentStepExecutor({
        service: service(),
        parseInput: parseHumanApprovalStepInput,
        onResult: (r) => {
    ...
    expect(captured!.record.status).toBe('approved')
    expect(captured!.output).toEqual({ kind: 'approval' })
