...
function baseJob(overrides: Partial<Job> = {}): Job {
  return parseJob({
    ...
    stepRunId: 'step_run_as_001',
    status: 'queued',
    idempotencyKey: 'workflow:run_001:step:anti-slop-review:v1',
...
function draftPayload() {
  ...
  return { articleDraftArtifact: draft.record, sourceStepRunId: 'step_run_as_001' }
}
...
describe('anti-slop-review-runner', () => {
  it('runs anti-slop-review through the durable runner and captures the review', async () => {
    ...
    const worker = createRunnerWorker({
      ...
      executor: createContentStepExecutor({
        service: service(),
        parseInput: parseAntiSlopReviewStepInput,
    ...
    expect(captured!.record.artifactType).toBe('anti-slop-review')
    expect(captured!.output).toEqual({ kind: 'artifact', artifactType: 'anti-slop-review' })
  ...
  it('fails the job when the upstream artifact is malformed', async () => {
    const repo = repository()
    const job = baseJob({
      jobId: 'job_as_bad_001',
      payload: { articleDraftArtifact: { not: 'an artifact' }, sourceStepRunId: 'step_run_as_001' }
    })
    repo.saveJob(jobRecord(job))
    const worker = createRunnerWorker({
      repository: repo,
