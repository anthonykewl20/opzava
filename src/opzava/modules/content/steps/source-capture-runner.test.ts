...
function service() {
  let n = 0
  return createSourceCaptureStepService({
...
describe('source-capture step runner', () => {
  it('runs source-capture through the durable runner and captures the artifact', async () => {
    const repo = repository()
    let captured: { record: { artifactType: string }; output: unknown } | null = null
    const job = baseJob({
      payload: { idea: VALID_IDEA, sourceStepRunId: 'step_run_sc_001' },
    })
    ...
    const worker = createRunnerWorker({
      ...
      executor: createContentStepExecutor({
        service: service(),
        parseInput: parseSourceCaptureStepInput,
        onResult: (r) => {
          captured = r as unknown as {
    ...
    expect(captured!.record.artifactType).toBe('source-capture')
    expect(captured!.output).toEqual({ kind: 'artifact', artifactType: 'source-capture' })
  ...
  it('fails the job when the manual input is malformed', async () => {
    const repo = repository()
    const job = baseJob({
      jobId: 'job_sc_bad_001',
      payload: { idea: {}, sourceStepRunId: 'step_run_sc_001' },
    })
    repo.saveJob(jobRecord(job))
    const worker = createRunnerWorker({
      repository: repo,
