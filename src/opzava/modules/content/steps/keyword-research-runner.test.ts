...
function service() {
  let n = 0
  return createKeywordResearchStepService({
...
describe('keyword-research step runner', () => {
  it('runs keyword-research through the durable runner and captures the artifact', async () => {
    const repo = repository()
    let captured: { record: { artifactType: string }; output: unknown } | null = null
    const job = baseJob({
      payload: { idea: VALID_IDEA, sourceStepRunId: 'step_run_kw_001' }
    })
    ...
    const worker = createRunnerWorker({
      ...
      executor: createContentStepExecutor({
        service: service(),
        parseInput: parseKeywordResearchStepInput,
        onResult: (r) => {
    ...
    expect(captured!.record.artifactType).toBe('keyword-research')
  ...
  it('fails the job when the manual input is malformed', async () => {
    const repo = repository()
    const job = baseJob({
      jobId: 'job_kw_bad_001',
      payload: { idea: {}, sourceStepRunId: 'step_run_kw_001' }
    })
    repo.saveJob(jobRecord(job))
