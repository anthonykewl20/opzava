...
import { createMockSourceCaptureProviderProfile, SOURCE_CAPTURE_OPERATION } from './source-capture-adapter'

export type SourceCaptureProviderCallInput = Readonly<{
  idea: IdeaIntake
  workflowRunId: string
  stepRunId: string
}>

export type SourceCaptureProviderCallDeps = Readonly<{
...
export async function runSourceCaptureProviderCall(
  deps: SourceCaptureProviderCallDeps,
  input: SourceCaptureProviderCallInput
  ...
  const request = parseProviderAdapterRequest({
    ...
    idempotencyKey: `workflow:${input.workflowRunId}:step:${input.stepRunId}:${SOURCE_CAPTURE_OPERATION}`,
    timeoutMs: 30_000,
