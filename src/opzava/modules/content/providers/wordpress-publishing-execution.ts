...
import { createMockWordpressPublishingProviderProfile, WORDPRESS_DRAFT_CREATE_OPERATION } from './wordpress-publishing-adapter'

export type WordpressPublishingCallInput = Readonly<{
  draftRequest: WordpressDraftRequest
  workflowRunId: string
  stepRunId: string
}>

export type WordpressPublishingCallDeps = Readonly<{
...
export async function runWordpressPublishingCall(
  deps: WordpressPublishingCallDeps,
  input: WordpressPublishingCallInput
  ...
  const request = parseProviderAdapterRequest({
    ...
    idempotencyKey: `workflow:${input.workflowRunId}:step:${input.stepRunId}:${WORDPRESS_DRAFT_CREATE_OPERATION}`,
    timeoutMs: 30_000,
    retry: { attemptNumber: 1, maxAttempts: 3 },
    input: input.draftRequest,
    requestSummary: { requestId: input.draftRequest.requestId },
    startedAt
  })
