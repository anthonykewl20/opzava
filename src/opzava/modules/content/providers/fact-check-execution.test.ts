...
import { createMockFactCheckProviderAdapter } from './fact-check-adapter'
import { runFactCheckProviderCall } from './fact-check-execution'
...
function deps(s: ReturnType<typeof sink>) {
  let n = 0
  return {
    adapter: createMockFactCheckProviderAdapter({ now: () => '2026-06-15T00:00:05.000Z' }),
    eventSink: s,
    newId: () => `id_${++n}`,
...
describe('fact-check-execution', () => {
  it('routes the fact-check call through the platform execution path', async () => {
    const i = inputs()
    const s = sink()
    const execution = await runFactCheckProviderCall(deps(s), {
      articleDraft: i.articleDraft,
      sourceCapture: i.sourceCapture,
      workflowRunId: 'run_001',
      stepRunId: 'step_run_fc_001'
    })
    expect(execution.result.status).toBe('succeeded')
    expect(execution.externalCallRecord.providerId).toBe('mock-fact-check')
  })

  it('emits external-call, cost, and audit operational events', async () => {
    const i = inputs()
    const s = sink()
    await runFactCheckProviderCall(deps(s), {
      articleDraft: i.articleDraft,
      sourceCapture: i.sourceCapture,
      workflowRunId: 'run_001',
      stepRunId: 'step_run_fc_001'
