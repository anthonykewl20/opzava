...
/**
 * Status-action registry — the deep module behind GET /api/status.
 *
 * The route used to be a ~800-line god handler: one HTTP GET switched on `?action=`
 * across seven inline orchestrators (system, dashboard, db stats, gateway, models,
 * health, capabilities), each carrying its own response shape, auth expectation, and
 * error handling. This module hides all of that behind one small interface:
 *
 *   - {@link STATUS_ACTIONS}: the action table (name -> {@link StatusAction}).
 *   - {@link getStatusAction}: the dispatcher's lookup.
 *
 * Each action is an adapter that declares whether it requires auth and carries a
 * `run(ctx)` that returns its full response body. The route is now a thin HTTP
 * adapter (parse action, enforce auth per the action's declaration, dispatch,
 * uniform error handling). Handlers are individually addressable and testable
 * through this interface rather than through the HTTP stack.
 *
 * Adding a surface (e.g. Engine-B workflow status, realtime-chat metrics) is a new
 * entry in the table, not an edit to a god switch — which is what makes this the
 * enabling seam for folding further surfaces into the "one dashboard".
 */

export interface StatusActionContext {
  /** Resolved workspace id from the authenticated viewer (1 for anonymous actions). */
  workspaceId: number
  /** The inbound request (some actions read headers/query, e.g. capabilities). */
  request: NextRequest
}

export interface StatusAction {
  /** False only for anonymous probes (health), which run before auth. */
  requiresAuth: boolean
  /** Returns the JSON-serializable response body for this action. */
  run: (ctx: StatusActionContext) => Promise<unknown> | unknown
}
...
async function getMemorySnapshot() {
  ...
  // Swap (cross-platform). Surplus to the legacy status-action callers, which
  // select only totalBytes/usedBytes/availableBytes/usagePercent; required by
  // the system-monitor surface, whose panel renders swap.
...
/**
 * The action table — the single registry of routed `?action=` surfaces.
 * To add a surface: add an entry here. Each entry owns its response shape and
 * declares whether it may run without an authenticated viewer.
 */
export const STATUS_ACTIONS: Record<string, StatusAction> = {
  overview: { requiresAuth: true, run: ({ workspaceId }) => getSystemStatus(workspaceId) },
  dashboard: { requiresAuth: true, run: ({ workspaceId }) => getDashboardData(workspaceId) },
  gateway: { requiresAuth: true, run: () => getGatewayStatus() },
  models: { requiresAuth: true, run: async () => ({ models: await getAvailableModels() }) },
  health: { requiresAuth: false, run: () => performHealthCheck() },
  capabilities: { requiresAuth: true, run: ({ request }) => getCapabilities(request) },
}

/** Look up a routed action by name. Undefined for unknown actions. */
export function getStatusAction(action: string): StatusAction | undefined {
