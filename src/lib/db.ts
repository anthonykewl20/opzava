...
import { registerOpzavaRunnerMigrations } from '../opzava/platform/runner/migrations';
...
export function logAuditEvent(event: {
  ...
  const db = getDatabase()
  // Auto-correlate the audit event to the active request when called inside a
  // withRequestContext-wrapped route, so the audit trail joins the log trail on
  // request_id (no call-site changes — the id is read from AsyncLocalStorage).
  const requestId = getRequestId()
  let detailJson = event.detail ? JSON.stringify(event.detail) : null
  if (requestId) {
    const merged = event.detail && typeof event.detail === 'object'
      ? { ...event.detail, request_id: requestId }
      : { request_id: requestId }
    detailJson = JSON.stringify(merged)
  }
  db.prepare(`
