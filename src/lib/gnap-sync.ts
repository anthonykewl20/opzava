...
function git(repoPath: string, args: string[]): string {
  try {
    return execFileSync('git', args, {
      ...
      env: {
        ...process.env,
        GIT_AUTHOR_NAME: process.env.GIT_AUTHOR_NAME || 'Opzava',
        GIT_AUTHOR_EMAIL: process.env.GIT_AUTHOR_EMAIL || 'opzava@localhost',
        GIT_COMMITTER_NAME: process.env.GIT_COMMITTER_NAME || 'Opzava',
        GIT_COMMITTER_EMAIL: process.env.GIT_COMMITTER_EMAIL || 'opzava@localhost',
      },
      timeout: 15_000,
...
export function syncGnap(repoPath: string): SyncResult {
  ...
  if (hasRemote(repoPath) && hasChanges(repoPath)) {
    try {
      git(repoPath, ['add', '.'])
      git(repoPath, ['commit', '-m', `Sync from Opzava at ${result.lastSync}`])
      git(repoPath, ['push'])
    } catch (err: any) {
