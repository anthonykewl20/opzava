...
| **Ask Admin Opzava** | The ADMIN-facing chat surface = the WebChat-parity port (program row #14; `chat.*` RPCs via the broker). Lives in the admin dashboard and is Tasks/platform-ops only. | **Ask Opzava** (user-facing); OpenClaw's native WebChat UI itself. |
