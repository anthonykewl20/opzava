...
2. **Use these skills:** `opzava-conventions` (always) · <specialized skills> · `tdd` · `no-mistakes` · `code-review`. Author any missing skill with `writing-great-skills` first.
...
5. **Definition of Done:** deliverables complete · acceptance passes · `tdd` tests + lint + typecheck green (`no-mistakes`) · local↔Dokploy parity intact · `docs/plan/EXECUTION.md` updated · committed on a branch off `development`.
