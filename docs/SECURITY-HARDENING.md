...
Comprehensive security hardening guide for Opzava and OpenClaw Gateway deployments.

## Production Hardening Quickstart

The fastest path from a fresh clone to a network-facing, approval-gated beta deploy.

```bash
# 1. Configure environment from the template (strong creds auto-generate if blank)
cp .env.example .env
bash scripts/generate-env.sh --force    # writes strong AUTH_PASS / API_KEY / AUTH_SECRET
# Edit .env: set MC_ALLOWED_HOSTS=opzava.anito.ai (your real hostname)

# 2. Terminate TLS at a reverse proxy (Caddy example — auto Let's Encrypt)
#    Caddyfile:
#      opzava.anito.ai { reverse_proxy 127.0.0.1:3000 }
#    caddy run --config Caddyfile
# In .env, set NEXT_PUBLIC_FORCE_HTTPS=1, MC_COOKIE_SECURE=1, MC_COOKIE_SAMESITE=strict, MC_ENABLE_HSTS=1

# 3. Boot the hardened profile (read-only-ish, HSTS, strict cookies, log rotation, internal net)
docker compose -f docker-compose.yml -f docker-compose.hardened.yml up -d

# 4. Audit the running instance
bash scripts/security-audit.sh          # expects PASS on creds/network/docker
```

### Validate live providers (do this before inviting users)

Live external actions (WordPress draft, Resend email) are exercised against mocks in CI but
must be validated once against real provider APIs before beta:

1. **Settings → Provider Connections** — enter WordPress site URL + application password, and
   Resend API key + sender identity. Values are stored as admin settings; secrets are never
   returned or logged.
2. Click **Test** on each — this performs a real reachability/credential check
   (`GET <site>/wp-json/` for WordPress, `GET api.resend.com/domains` for Resend) via
   `POST /api/connections/test`.
3. Optionally run one real end-to-end: start a content run (`/api/ops/runs`, draft-only) and a
   campaign send (`/api/campaigns/[id]/run`) with approval gates on. Confirm a WordPress draft
   appears and one Resend email delivers.

> Beta operating mode stays **approval-gated and draft-only**: no auto-publish, no auto-send.
> Every external action is recorded as an `ExternalCall` + `CostEvent` + `AuditEvent`.
...
Opzava uses a host allowlist in production:
...
Opzava sets these headers automatically:
...
- Audit third-party skills before installing (Opzava's Skills Hub runs automatic security scans).
...
- Use Opzava's diagnostics API to monitor gateway health.
...
Opzava logs a security event (`global_api_key_used`) every time the global API key is used. Monitor these in the audit log:
