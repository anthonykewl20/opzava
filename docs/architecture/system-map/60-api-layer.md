...
> Marks: 🔎 pass-1 research · ✅ verified · ⚠️ parity flag.
...
and `auth/login`/`auth/google` (rate-limited instead).
...
5. `campaigns/[id]/run` is the **only** opzava route performing live external I/O (real Resend send, F1);
   failures route to runner dead-letters.
