...
An upstream bump is a deliberate operation, never an image-tag change: scratch-clone upstream outside the repo, `git diff` old pin to new tag, review the diff against PATCHES.md so every rung-3 patch re-applies or is upstreamed or retired, apply the new working tree, update the pin, rebuild the image, and prove it before committing — provider connect still works (patch 3, `onboard --credential-stdin`, is not upstreamed and an image without it fails closed), the broker and worker reconnect, and whatever else the bump touched is driven on the real local stack per the verify posture (`mainframe/UPSTREAM.md` bump procedure; `docs/adr/ADR-016-mainframe-tracked-fork.md` decision 1). All five PATCHES.md rows are rung-3, so all five are re-applied and re-verified, not only the riskiest.
