...
**Goal:** Extract Aegis as a deep module (`core/reviews`), split the `task-dispatch.ts` god-module so dispatch enqueues and the runner executes, make the runner the canonical path for all three engines with the `task_id` FK, and dedupe the client adapters into a shared module (split C4 into C0 minimal-early + C4 deep to fix v1's backwards dep).
