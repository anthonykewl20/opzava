...
| D1–D7 | Device-auth + persistent reliable connection | 🟡 (D1+D2+B1b+D3 done) | D1 ✅ migration 058. **D2 ✅** core/auth module (rotation guard). **B1b ✅** resolveDeviceToken wired into getUserFromRequest (device tokens resolve via the cascade; src/lib/auth added to the engine-boundary allowlist). **D3 ✅** POST /api/auth/device/token (refresh grant) — rotates via D2, maps outcomes to HTTP (rotated→200+pair, already_rotated→409, invalid_grant/reuse→401), grant-type-agnostic for the future device_code/HTTP-MCP grants. route.test 5/5. D4–D7 ⬜ (device-code issuance + /device approval + DB-backed rate limits + client + docs). |
