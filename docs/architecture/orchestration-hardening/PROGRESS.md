...
**Handoff gate: `pnpm test` 2326/2326 ✅ · `pnpm test:governance` ✅ (0 fail) · `pnpm typecheck` ✅ — zero regressions.**
...
- `20123d0` A2 — transactional capture spine with post-commit broadcasts (✅; the previously-deferred tx-wrap, via a broadcast/write decoupling in db_helpers)
- `d6c9b99` D1 — migration 058 device_tokens + oauth_device_sessions + denylist (✅)
- `36629e1` D2 — core/auth device-token module with server-safe rotation guard (✅)
...
| B1b | FUSED resolveDeviceToken cascade branch | ✅ | _pending_ | auth 25/25 + engine-boundary 3/3 + typecheck. `resolveDeviceToken` wired into `getUserFromRequest` (after `agent_api_keys`, before the plugin hook — agent keys win precedence; device tokens share one authz path). `src/lib/auth.ts` added to the engine-boundary sanctioned-crossings allowlist (the deliberate device-auth seam). Optional `MC_DEVICE_INSTANT_REVOKE` denylist check included. getUserFromRequest device-token integration test = follow-up. |
| D1–D7 | Device-auth + persistent reliable connection | 🟡 (D1+D2 done) | D1 ✅ migration 058 (tables). **D2 ✅** `src/opzava/core/auth/` — contracts + crypto + device-code + token-service (issueForDevice/resolveDeviceToken/rotate/revoke), dependency-injected (pure core/, layering-clean). Server-safe rotation guard: INSERT-new-row-per-rotation (preserves the old hash for reuse detection) + grace-window disambiguation (concurrent loser → already_rotated, NOT revoked; stale replay → reuse_detected → family revoke). Device tokens never admin (capped); explicit workspaceId required. auth.test 8/8 + architecture 5/5. D3–D7 ⬜ (endpoints/client/docs — large). |
