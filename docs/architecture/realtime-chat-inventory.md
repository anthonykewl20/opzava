...
| GET | `/api/ops/chat-doctor` | admin | Chat health doctor following the current `/api/openclaw/doctor` response shape. Checks write-atomicity, idempotency-index, resync/replay wiring, ephemeral guard, drain handler | none | `DoctorStatus[]` or aggregated `{level,category,healthy,summary,issues[],canFix}`. Headers `Cache-Control:no-store` | `src/app/api/ops/chat-doctor/route.ts` (CREATE) + `src/lib/chat-doctor.ts` (CREATE, mirror the route-local status shape in `src/app/api/openclaw/doctor/route.ts`) | Phase-1 |
...
**DoctorStatus shape** (route-local in `src/app/api/openclaw/doctor/route.ts`): `{ level: 'healthy'|'warning'|'error', category: 'config'|'state'|'security'|'general', healthy: boolean, summary: string, issues: string[], canFix: boolean }`. OpenClaw doctor is now a Docker sidecar health probe; there is no local `openclaw doctor` parser module.
...
Integration target: `src/lib/chat-doctor.ts` (CREATE) aggregating `DoctorStatus[]`, mirroring the route-local response shape at `src/app/api/openclaw/doctor/route.ts`. Endpoint `src/app/api/ops/chat-doctor/route.ts` (CREATE, admin role).
...
**Endpoint contract:** `GET /api/ops/chat-doctor` returns `DoctorStatus[]` (or aggregated `{level,category,healthy,summary,issues[],canFix}`). Headers `Cache-Control:no-store`.
