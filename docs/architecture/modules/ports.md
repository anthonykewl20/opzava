...
The key invariants it enforces are: every operation returns `Result<T>` from `@opzava/shared-kernel` (no thrown-control-flow crosses a seam), every actor carries a scoped principal (`tenantId`/`orgId`/`workspaceId`/`userId`/`roleKeys`), and no vendor type leaks through the surface. Principal shapes vary by seam (e.g. `AuthorizationSubject` and `ConnectionProvisioningPrincipal` carry their own subsets); the narrowest by design is `OpenClawActingPrincipal`, reduced to `tenantId` alone by ADR-018 (#194) so the broker verifies only the tenant and no other identity field crosses the web→broker boundary.
