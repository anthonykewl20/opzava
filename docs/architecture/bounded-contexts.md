...
The owning ADRs come from the same map and are confirmed against `docs/adr/` (ADR-001 through ADR-016 all exist on disk).
The owning package or app comes from the on-disk monorepo layout: packages are `adapters`, `config`, `crm`, `identity-access`, `ports`, `project-management`, `runtime-control`, `shared-kernel`; apps are `gateway-broker`, `mcp-server`, `web`, `workers`.
...
| Platform-Ops | `apps/workers` and `apps/web` (admin ops surfaces, `debug.html` and `settings.html` touchpoints) | Opzava Postgres: host placement, route health, repair or deprovisioning state, ADMIN board projections, remediation audit (`ARCHITECTURE.md:60`) | ADR-002, ADR-013, ADR-015 | Partially built: Connections and settings surfaces in Slice 3.7; debug and full ops are P8 (`EXECUTION.md:41`, `:466` P8 ADR refs) |
...
| CRM | `packages/crm` (`@opzava/crm`) | Opzava Postgres: contacts, accounts, deals, pipelines, activities, tickets, segments, consent, `ChannelIdentity`, merge and erasure audit (`ARCHITECTURE.md:66`) | ADR-011 | Thin core built in Slice 3 (Contact, Account, Deal, Ticket, versioned Pipeline/Stage); channel ingest, merge, erasure deferred to P4 (`EXECUTION.md:227` Slice 3 done, `:249` deferred, `:353` P4 not started) |
| External Channels | `apps/gateway-broker` ACL adapter to OpenClaw; no Opzava domain package | Opzava Postgres for channel correlation and projections; OpenClaw Gateway for provider connectivity, credentials, external conversation runtime, sends and receives (`ARCHITECTURE.md:67`) | ADR-011, ADR-003 | Partially built: provider connect and Connections surface in Slice 2.5 and 3.7; Slack, WhatsApp, Gmail, email flows deferred to P8 (`EXECUTION.md:206`, `:41`, `:449` P8 not started) |
...
| Finance and Billing | No dedicated package yet; `BillingPort` declared in `packages/ports` | Opzava Postgres: plans, subscriptions, invoices, usage meters, meter events, entitlement state, quota policy, dunning; `BillingPort` is a null-adapter seam until external monetization (`ARCHITECTURE.md:69`) | ADR-014 | Deferred: ADR-014 accepted as deferred; `BillingPort` null adapter; costs and metering land in P6 (`EXECUTION.md:399` P6 not started) |
| Notifications and Admin-Observability | No dedicated package yet; `ErrorCapturePort` declared in `packages/ports` (`error-capture.ts`) | Opzava Postgres: notifications, error groups, error events, alert routes, remediation actions, tenant-visible incident projections (`ARCHITECTURE.md:70`) | ADR-013 | Partially built: Issues page and health pill in Slice 2.5 and 3.5; incident pipeline, alert routes, remediation loop deferred to P7 (`EXECUTION.md:208`, `:423` P7 not started) |
...
CRM depends on Project Management today and on External Channels at P4.
Slice 3 reused the PM aggregate and RLS pattern for CRM records (`EXECUTION.md:247` bounded contexts CRM, Project Management, Identity and Access).
P4 adds External Channels, AI Workforce, Runtime-Control, Knowledge Management, and Internal Collaboration as CRM dependencies (`EXECUTION.md:373`).

External Channels depends on Gateway Runtime for provider connectivity and on CRM for `ChannelIdentity` resolution.
Channel plugins emit sender and conversation events through the ACL; Opzava resolves `ChannelIdentity(channel, externalId)` to a Contact or shell (`docs/plan/capability-parity.md:105`, `ARCHITECTURE.md:67`).
...
Incident inputs come from `logs.tail`, `diagnostics.stability`, `tasks.list`, Workboard diagnostics, and app errors, projected as Opzava `ErrorGroup` and ADMIN cards (`docs/plan/capability-parity.md:120`, ADR-013).
...
CRM is hybrid: Contact, Ticket, Activity, and Consent are Opzava-owned; external conversation transcripts and provider connection are OpenClaw-owned through the ACL (`docs/plan/capability-parity.md:103-105`).
...
Each Opzava-owned context that has been started gets one bounded-context package with its own Drizzle and Postgres ownership (`ARCHITECTURE.md:15`): `@opzava/identity-access`, `@opzava/project-management`, `@opzava/runtime-control`, `@opzava/crm`.
