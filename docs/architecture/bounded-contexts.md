...
CRM is deferred: its former implementation was removed on 2026-07-15 (GitHub issue #200). The retained ADR-011 dependency design returns only with the future user-side dashboard.

External Channels depends on Gateway Runtime for provider connectivity. Future channel flows may use ADR-011 `ChannelIdentity` resolution when deferred CRM returns with the user-side dashboard.
...
CRM is deferred: its future user-side-dashboard rebuild will own customer truth while external conversation runtime remains behind the ACL (ADR-011; GitHub issue #200).
