...
- `OPENCLAW_GATEWAY_URL`: admin WebSocket URL, normally `ws://openclaw-platform-gateway:18789` in
  compose.
- `OPENCLAW_OPERATOR_DEVICE_TOKEN`: paired operator device token for the worker container. Local
  compose maps this from `WORKER_OPENCLAW_OPERATOR_DEVICE_TOKEN`. This token stays on the worker and
  is never passed to the gateway-broker.
- `OPENCLAW_OPERATOR_SCOPES`: optional comma- or space-separated scopes to request at Gateway
  ...
  only for workers allowed to mutate Gateway config. Local compose maps this from
  `WORKER_OPENCLAW_OPERATOR_SCOPES`.
...
2. Pair the worker operator device and put only the resulting
   `WORKER_OPENCLAW_OPERATOR_DEVICE_TOKEN` plus the matching private key env in `.env`.
...
proxy enables the container, network, and exec verbs needed for Gateway-local onboarding flows and
keeps build, images, volumes, swarm, secrets, and services endpoints disabled. Do not attach the
gateway-broker or web service to this proxy.
...
Provider connects run through the provisioning worker. API-key connects run the Gateway onboarding
path asynchronously and then verify the provider through the same snapshot sources. Model-provider
device-flow and setup-token connects execute the Gateway-local auth flow through the worker-only
Docker runtime and return only the verification URL, user code, or authorize URL needed by the
browser. GitHub connects use GitHub device flow and store the access token in `SecretsVaultPort`;
status is validated against `GET https://api.github.com/user` and the OAuth scopes response header.
...
The Connections page treats `agents.defaults.model.primary` as the single main-orchestrator model.
Selecting a different connected provider posts through `/api/connections/orchestrator/set-main`,
which forwards to `/internal/connections/orchestrator/set-main`, requires a connected provider with
a routable model, patches the Gateway primary model, and rebuilds subagent delegation so the
previous lead remains available as a subagent.
