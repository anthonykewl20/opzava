...
Get from zero to a working agent loop with nothing but Opzava and `curl`. No gateway, no OpenClaw, no extra dependencies.

## Prerequisites

- Opzava running (`pnpm dev` or Docker)
...
Agents can self-register via the API. This is how autonomous agents announce themselves to Opzava:
...
Heartbeats tell Opzava your agent is alive. Without them, agents are marked offline after 10 minutes:
