...
2. **Proposal, Dependency Edge (`DependencyEdge`), and Execution Lease (`ExecutionLease`) are
   ...
   spans two DevTickets; a lease belongs to Runner admission and has a higher-churn, separately
   retained ledger.
...
6. **Claim and start are a two-phase application saga.** The first transaction atomically assigns
   the actor, reserves Runner capacity, and grants a fenced lease while the Card remains in Todo. A
   nonempty secret-ref set enters `credential_provisioning`; only a zero set or fully activated set
...
| Lease Credential Access Grant (`LeaseCredentialAccessGrant`) | Runner/Execution Admission-owned lease ID, exact Ready-approved named-secret ref IDs, broker/local grant refs (never values), status (`pending`, `active`, `revocation_pending`, `revoked`, `provision_failed`), versions, and activation/revocation confirmation refs                                                                                                                                                                                                                             | the underlying secret/credential, Secrets authority, or reusable reviewer/successor access                                                                                                                 |
...
because Runner fencing, heartbeat, and receipt ordering have a different authority, write rate, and
retention policy. GitHub Issue Binding is the unique ownership registry for provider identity rather
than a field inferred from a mirror projection. The application service coordinates these records
transactionally where an invariant crosses boundaries; no aggregate calls GitHub, Slack, or a Runner
adapter.
...
5. **Integration module:** GitHub managed-mirror outbox/inbox/dedupe/conflict records. It consumes
   `DevBoardMirrorPort`; it cannot mutate the DevTicket except by an authenticated command request.
...
`ReconcileRunnerContainment` remains the sole process/worktree release owner for an ordinary Blocked
containment (or the already-owning Material Revision finalizer remains sole where applicable).
...
- Every command declares an expected-version set for **every mutable record it may touch or rely on
  ...
  set, Runner admission, active admission wait/retry, Review Handoff, material/stop reviewer
  containment requirement/inbox proof, Review Exit Containment Proof, global Review WIP counter,
  Sprint reservation, GitHub Issue Binding, Absolute Stop, lane queue/archive membership, and
  exclusive-resource reservation as applicable. Credential provisioning loss also names the active
  Revision/Material Revision Interruption, Pre-Start Admission Loss, exact claim/grant/tunnel
  versions, provision/start outbox state, and deterministic no-start/no-process proof inputs. Signed
  start rejection names those records plus Start Rejection Containment. Immutable version IDs/hashes
  are still named explicitly. Missing, extra, or mismatched expected state is a concurrency conflict
...
- Lock order is fixed across commands: tenant/workspace policy and Absolute Stop scope; exact
  ...
  (`ClaimAdmissionDeferred` first by ID, then Review retry intent); Runner admission/Claim Attempt
  arbitration, then Pre-Start Admission Loss and Start Rejection Containment rows in that order;
  underlying named secret-reference/credential records, then lease-scoped credential-access
  authority/grant rows, then local preview-tunnel grant/authority rows, each in sorted ID order;
  exclusive resources in sorted key order; lane queues in canonical lane order; command
  receipt/outbox append points. Absolute Stop, Ready, dependency, Sprint, Review, Runner, and both
  retry paths use this same order and never acquire records in caller order.
...
- Claim preparation declares the DevTicket, selected Runner admission row, global Review WIP
  counter, exact Sprint Plan/member/reservation where relevant, and conflicting exclusive-resource
  reservations, then acquires every applicable row only in the canonical total order above. Unique
  partial constraints allow at most one live Claim Attempt/Execution Lease per DevTicket, one
  current fencing token per lease, and one admitted Sprint item. Review WIP at three defers the
  claim without touching admitted leases.
...
resolution/supersession, and lane/queue facts; the Runner execution ledger owns terminal containment
proof and release.
...
pending grant; the final required confirmation enqueues Runner start exactly once. Stale, fenced,
stopped, or ref-revoked confirmation is recorded stale and atomically creates/reuses
...
Finalize/Abort. Broker facts stay in the integration inbox; Runner ledger owns local grant
lifecycle.
...
every grant/tunnel confirmation. If final activation/start enqueue wins the common locks first,
these pre-start commands are ineligible: provider/expiry/cancellation facts become stale
...
state has no Runner start outbox, accepted start marker, or process registration for this claim.
...
- Workflow/activity: `ProposalSubmitted`, `ProposalAccepted`, `ProposalMerged`, `ProposalRejected`,
  ...
  `ClaimStartFailed`, `ClaimAdmissionDeferred`, `ClaimAdmissionRetried`, `ClaimAdmissionCancelled`,
  `ClaimAdmissionSuperseded`, `RevisionAcceptedPendingApplication`, `RevisionApplied`,
  `RevisionRejected`, `RevisionAbortedAfterAcceptance`, `MaterialRevisionInterruptionAborted`,
  ...
  `AgentWorklogLinked`, `AbsoluteStopOpened`, `AbsoluteStopReviewContainmentRequired`,
...
- Runner: `ClaimPrepared`, `LeaseGranted`, `LeaseStarted`, `LeaseFenced`, `LeaseReleased`,
  `LeaseCredentialAccessReserved`, `LeaseCredentialAccessActivated`,
  ...
  `ExecutionStartReceiptRecorded`, `ExecutionStartRejected`, `RunnerReceiptAccepted`,
  `RunnerReceiptRejected`, `CheckpointAccepted`, `ExecutionStartDeadlineElapsed`, `StartExpired`,
  `ExecutionLossDetected`, `RunnerContainmentRequested`, `RunnerContainmentConfirmed`,
  `RunnerContainmentConfirmationRejected`, `PreStartAdmissionLossOpened`, `PreStartNoProcessProven`,
  `PreStartAdmissionLossFinalized`, `StartRejectionContainmentOpened`,
  `StartRejectionContainmentFinalized`, `MaterialRevisionProvisioningLossObserved`,
  `MaterialRevisionStartRejected`, `PreviewTunnelAuthorityRevoked`,
  `MaterialRevisionInterruptionPrepared`, `MaterialRevisionInterruptionFinalized`,
  `ReviewContainmentProofRecorded`, `ReviewExitContainmentProofRecorded`, `ContainmentReconciled`,
  `RunnerDisconnected`, `RunnerReconciled`, `AgentWorklogAppended`, and
  `AgentWorklogCorrectionAppended`.
...
| Comments and worklogs                                                   | —                                                         | only Secret-Safe Ingress-passed immutable human records/corrections; suspected-secret raw bodies never persist                                                      | only Secret-Safe Ingress-passed signed agent records; safe hash/ref links rejected/quarantined input                           | #231-owned append/correction intents only after ingress pass; safe security notification                               |
...
| Validate/approve/invalidate Ready                                       | rationale plus versioned validator inputs/result artifact | authoritative approval/carry-forward/invalidation and lane result                                                                                                   | fence/release refs if active work invalidated                                                                                  | managed contract/status write intent                                                                                   |
| Apply non-semantic correction to Approved/Queued/Active Sprint member   | exact Revision/classification/equivalence rationale       | new contract/Ready Approval plus exact Sprint Plan Binding Carry-Forward                                                                                            | immutable Execution Binding Carry-Forward for active/Blocked/Review lineage                                                    | managed contract/status worklog intent                                                                                 |
| Apply non-semantic correction to Draft Sprint member                    | exact Revision/classification/equivalence rationale       | atomic Draft Plan/member contract-ref update; no Sprint approval carry-forward                                                                                      | execution equivalence only if applicable                                                                                       | managed contract/status worklog intent                                                                                 |
| Approve Ready and request start                                         | validation/rationale refs only                            | Ready Approval, Todo lane, ClaimRequested bound to original requester/source, intended claimant, auth version, Runner capability, and exact gates                   | claim starts only after worker reauthorization/revalidation; drift cancels/supersedes                                          | internal claim-request outbox; no duplicate or worker identity substitution                                            |
| Needs Human Approval Request lifecycle                                  | request rationale and policy-evaluation inputs            | authoritative open/approve/reject/expire/revoke/consume facts                                                                                                       | fence ref only when the bound gate pauses execution                                                                            | Slack/GitHub request and decision delivery attempts                                                                    |
| Absolute Stop lifecycle                                                 | safe evidence/remediation rationale refs                  | authoritative open/resolve, gate, local credential/tunnel revocation, lane effects; already-Blocked reuses one owner; finalized Review retains handoff/WIP          | ordinary Runner request/proof or distinct #229 Reviewer/Docker/tunnel/evidence proof; one existing release owner where present | durable Runner/#229/credential/tunnel intents, confirmations, and health reconciliation                                |
| Fence possibly live execution for Block or Absolute Stop                | safe reason/evidence refs only                            | Blocked Episode/Absolute Stop and unresolved containment cross-ref                                                                                                  | one idempotent Runner Containment Request bound to lease/fences/process/worktree/action/nonce, then authenticated confirmation | Runner checkpoint+stop/quarantine delivery and retry; credential/tunnel confirmations remain separate                  |
| Finalized-Review material Revision containment                          | Revision rationale and exact review binding               | containment-required/accepted refs; finalized Review Handoff unchanged; Finalize owns Review→Backlog/WIP                                                            | #229-authenticated Review Containment Proof plus interruption release                                                          | idempotent #229 containment request and confirmation inbox                                                             |
...
| Abort material interruption                                             | authorized abort disposition/rationale artifact           | authoritative abort; preserved approval + Todo or invalidation + Backlog; optional Blocked resolution/supersession; Absolute Stop unchanged                         | terminal containment proof and release                                                                                         | optional safe status/mirror intent                                                                                     |
| Add/remove dependency                                                   | rationale + contract Revision ref                         | edge/version and invalidation result                                                                                                                                | fence ref only if Revision stops active work                                                                                   | managed dependency/status intent/conflict refs                                                                         |
| Claim/start/loss/block/recover/submit/admit Review                      | planning only when a policy decision/Revision occurs      | claim/lane/WIP plus Review Handoff prepared/cancelling/finalized/cancelled/superseded/failed facts and candidate rebound; WIP-full Submit creates no Review Handoff | credential lifecycle, frozen candidate, lease/fence/containment/release facts                                                  | provision/revocation/stop confirmations, Review retry/Slack; fresh reviewer tunnel only after finalized Review Handoff |
| Open/finalize Pre-Start Admission Loss from provider/expiry/cancel/loss | —                                                         | Todo unchanged, source-linked failed-request reason, claim-created assignment cleared only at safe Finalize                                                         | shared no-start/no-process proof, failed claim/lease, held then released capacity/worktree                                     | provision/start suppression and grant/tunnel revocation confirmations                                                  |
| Open/finalize Start Rejection Containment                               | —                                                         | Todo unchanged, signed-rejection failed-request reason, claim-created assignment cleared only at named Finalize                                                     | signed no-process proof, failed claim/lease, held then released capacity/worktree                                              | grant/tunnel revocation intents and confirmations                                                                      |
| Provision loss/start rejection subordinated to material Revision        | Revision rationale remains owning decision                | one interruption remains `AcceptedPendingApplication`; no Pre-Start/Start Rejection lifecycle or independent lane result                                            | source identity plus deterministic no-start/no-process proof appended to owning interruption                                   | existing interruption revocation/confirmation set reused; no duplicate finalizer                                       |
| Block cause subordinated to material Revision                           | Revision rationale remains owning decision                | one `MaterialRevisionBlockCauseAppended`; lane/WIP/claim unchanged; no `BlockedOpened` or Blocked Episode                                                           | cause refs appended to existing interruption; no new request/finalizer/reconciler                                              | reuse existing containment deliveries; no duplicate outbox; optional notification links the event                      |
| GitHub webhook or mirror write                                          | — unless it proposes a governed Revision                  | accepted command only if policy admits one                                                                                                                          | pause/fence cross-ref if conflict blocks execution                                                                             | delivery dedupe, provider fact, attempts, confirmation, conflict/health                                                |
| Archive/restore                                                         | decision/reason                                           | lifecycle fact, Ready archive-deactivation, and safe-lane result                                                                                                    | required fence/release refs                                                                                                    | provider archive/restore disposition intents                                                                           |
...
| `AppendWorklog` / `CorrectWorklogByAppend`     | authorized actor or signed active Runner source, exact DevTicket/worklog stream and lease/fence where applicable, Secret-Safe Ingress pass                                                                                                              | source/delivery + client worklog ID + body/evidence hash                                                              | append immutable worklog or correction with `correctsRecordId`; suspected secret persists no receipt/body/activity/outbox and invokes safe stop path                                                                                  | Runner/activity own passed record only; security owns safe rejected/quarantine ref; #231 owns post-pass sync intent              |
...
| `ApproveReadyAndStart`                         | same as approval plus original verified requester/source, intended claimant, auth policy/version, selected Runner enrollment/capability refs, exact target/gates, and stable claimRequestId                                                             | approval identity + claimRequestId                                                                                    | Ready Approval + Todo + fully bound idempotent ClaimRequested outbox atomically; replay returns both IDs                                                                                                                              | activity owns approval/lane/request; internal outbox owns retry; Runner ledger begins later                                      |
| `DispatchClaimRequested`                       | active ClaimRequested; rederived current requester/claimant/source authorization; exact auth, Runner enrollment/capability and target/gate refs; stable worker key                                                                                      | claimRequestId + worker attempt                                                                                       | consume and invoke ClaimAndStart only after full reauthorization/revalidation; auth/source/enrollment drift cancels, material target/gate drift supersedes, both without claim/lease/resources                                        | activity owns consumed/cancelled/superseded request; Runner begins only after admitted claim                                     |
...
| `ArchiveDevTicket`                             | safe archive conditions; exact assignment and Pre-Start/Start Rejection versions; no containing record, live Review Handoff, claim/lease, Blocked Episode, nonterminal Sprint membership, or unreconciled authority                                     | DevTicket/version + Ready Approval/assignment/archive reason/Sprint version                                           | clear active Ready and current assignee/preassignment while preserving histories; archive/remove queue; Review remains separately gated                                                                                               | planning reason; activity archive/assignment/queue; Runner/integration reconciliation                                            |
...
| Final required provision confirmation succeeds                                                                                    | atomically activate last grant, arm deadline, enter `start_pending`, and enqueue Runner start once                                                                                                                                                      | no deadline during provisioning; one start outbox and one state transition                                       |
| Final provision confirmation races trusted-clock provisioning expiry                                                              | canonical selected-owner locks choose: confirmation-first enters `start_pending`; expiry-first appends to an owning Material Revision Interruption or explicit-none opens/reuses Pre-Start Admission Loss and makes confirmation stale/revocation-bound | exactly one lifecycle/finalizer; never start plus provisioning-loss containment                                  |
...
| Two claimants race                                                                                                                | exactly one Claim Attempt/lease/capacity reservation wins; nonempty not-yet-active secret refs enter `credential_provisioning`, while zero required grants or already activated grants enter `start_pending`                                            | loser gets deterministic conflict; winner state matches its exact grant set                                      |
