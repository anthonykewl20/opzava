...
- A **Human/agent principal** owns request intent and current Opzava authorization; it is not proof
  of machine possession or process state.
- A **machine enrollment** owns the local controller public key, owner, admitted capabilities,
  build/version, and revocation; it is not actor authorization or DevTicket assignment.
- A **Runner** is the selected execution location/control endpoint; it is not an agent/model or
  OpenClaw Node.
- A **Harness adapter** translates Codex/Claude/OpenClaw launch and normalizes observations; it is
  not workflow authority.
- A **Harness session/thread** is provider-native conversational/runtime continuation; it is not an
  Execution Lease or worktree identity.
- An **Execution Lease** owns DevTicket/Ready/Runner/fence/capacity authorization; it is not a
  provider login or Review authority.
- A **Slack/GitHub/OpenClaw identity** is an external transport/provider fact; it is not Opzava RBAC
  without current mapping and authorization.
