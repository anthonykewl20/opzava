...
Status: **Locked foundation — approved through the 2026-07-15 grilling session**
...
| DBF-142 | Auto-start preflight requires approved Goal/Plan, every member Ready, external dependencies Done, GitHub healthy/synchronized, runner/Reviewer online, local Docker healthy, named secret refs available, and no absolute stop. |
| DBF-143 | The Active Sprint permits exactly one implementation DevTicket In Progress. Once it enters Review, the execution agent may claim the next ordered Todo item.                                                                    |
| DBF-144 | Review WIP limit is three. At three, implementation pauses before claiming more and Slack notifies the Admin.                                                                                                                   |
...
| DBF-191 | An Active Sprint owns a fenced implementation slot, not the entire machine. Ordinary manual work queues unless the Sprint is safely paused, checkpointed, and a deliberate lease decision is made. P0 interruption uses the same governed preemption.                                                                                 |
...
- Broad parallel Sprint implementation beyond the locked single implementation slot and Review WIP
  three.
