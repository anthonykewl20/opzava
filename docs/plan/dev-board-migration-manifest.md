...
| `docs/plan/dev-board-foundation-decisions.md`            | Activated locked foundation plus the prepared WF-229 amendment set                                     | Canonical fine-grained decision source only for activated text. WF-229-amended legacy rows DBF-029, DBF-089, DBF-129–134, DBF-143–144, DBF-174, DBF-205–206 and new DBF-250–275 remain inactive until reviewed landing, the exact #229 evidence comment and #229 closure, and #228 designation of the exact landed memo plus governing-doc revisions. |
...
| `docs/plan/research/wf236-releases-gate-contract.md`     | Release aggregate, build-once, staging/production, approval, rollback, evidence, and Incident boundary | Resolved by closed #236 and current for #237 synthesis while map #228 is open. Canonical docs project the accepted amendment; it is not an implementation claim.                                                                                                                                                                                      |
...
### Prepared Wayfinder input

| Artifact                                           | Named map use                                                                                                              | Disposition                                                                                                                                                                                                                                                                                             |
| -------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `docs/plan/research/wf229-review-gate-contract.md` | #229 pre-Review Deepening Module admission plus independent Review Gate resolution for #228 and final synthesis under #237 | **Prepared inactive candidate.** It becomes a Current Wayfinder input only after reviewed landing, the exact #229 evidence comment and closure, and #228 designation of the exact landed memo plus governing-doc revisions. Mere issue linkage or this prepared row grants no implementation authority. |

At the Review boundary, #230's pre-#229 `ReviewMergeAuthorization` umbrella was a
command-precondition placeholder, not a persisted record schema. This prepared candidate
mechanically replaces it in the command-model memo with separate Review Exit Containment
Requirement/Proof, Provider Result Deepening Capture, Mark Pull Request Ready Request, Convert Pull
Request to Draft Request, their provider confirmations and singleton per-PR mutex, discriminated
Merge Authorization Requirement, conditional Merge Authorization Request/Authorization, Merge
Request/Provider Merge Attestation, Post-Merge Remediation Wait Edge, and External Result
Reconciliation Authorization Request/Authorization records, plus the External Result Review
Preparation and External Result Adoption Intent transition records. It also extends #230's command
catalog with generation-fenced successor handling, `AcceptReviewExitContainment`,
`FinalizeStoppedReviewAttempt`, `EvaluateMergeAuthorizationRequirement`,
`BlockExternalResultForRemediation`, and `ReconcileExternalResultDone` alongside
`ReviewChangesRequested` and `AdmitDone`. These refinements remain inactive until reviewed landing,
the exact #229 evidence comment and closure, and #228 designation of the exact landed memo plus
governing-doc revisions; #237 must carry the final chronological record split, allowed-command
matrix, and idempotent lock/consumption rules into the implementation graph.

The candidate also makes the Deepening Module Artifact a pre-Review admission dependency for every
code-producing source. Ordinary implementation binds its final checkpoint; an observed provider
result instead uses a source-read-only Provider Result Deepening Capture with no fabricated
implementation checkpoint and complete worktree/process/grant containment. A valid non-code result
uses a version-bound non-code fact plus explicit Capture/artifact/receipt absence. #237 must
preserve the source-discriminated immutable governed Docs binding, signed Runner receipt, Dev Board
applicability/current pointer, preflight/security-scan generations, stale/false-non-code terminal
receipt paths, and prohibition on treating the artifact as Review evidence or Pass.

### Target vocabulary for dependent specs after activation

Dependent specs must not adopt these prepared #229 terms as current authority until reviewed
landing, the exact #229 evidence comment and closure, and #228 designation of the exact landed memo
plus governing-doc revisions all complete. After that activation, every dependent spec uses the
vocabulary below; #237 still owns final synthesis into executable implementation tickets.
...
- Deepening Module Artifact: immutable governed Docs pre-Review architecture record for every
  code-producing source, with `pre_merge` candidate/implementation-receipt and `provider_result`
  provider-tree/capture-receipt variants, selected by an exact source-discriminated Dev Board
  applicability/current pointer; never Review evidence or Pass.
- Provider Result Deepening Capture: durable local source-read-only prepare/run/contain/finalize
  lifecycle for an authenticated code-producing provider tree, producing the provider-result
  artifact without an implementation checkpoint or source/provider mutation; non-code uses an exact
  version-bound fact plus explicit Capture/artifact/receipt absence instead.
- Review Attempt: one durable operational lifecycle for one source-snapshot/manifest identity; a
  Queued Attempt may exist before its optional launch-time Review Attempt Snapshot commits, and its
  terminal decision/history is immutable after cutoff and containment.
- Review Attempt Snapshot: one immutable discriminated binding of a pre-merge Frozen Implementation
  Candidate or provider-result Post-Merge Review Snapshot to current Ready, Reviewer/grants,
  manifest/policy, local Runner, Docker, and variant-required GitHub facts.
- Review Check Manifest: deterministic versioned mandatory checks for one Review Attempt.
- Docker Review Lease: exclusive fenced shared-local-stack authority, separate from Review WIP.
- Review Evidence Package: immutable secret-safe decision-time proof for one exact Review Attempt;
  later authorization, provider, and completion facts are linked records.
- Mark Pull Request Ready Request: system-owned GitHub saga with authenticated provider
  reconciliation and a singleton tenant/workspace/installation/repository/PR mutex shared with draft
  conversion and merge.
- Convert Pull Request to Draft Request: separate system-owned GitHub saga with authenticated
  provider reconciliation and that same singleton per-PR mutex.
- Post-Merge Remediation Wait Edge: Blocked-Episode-owned directed wait relation included in every
  dependency-cycle check until successful external-result Review resume closes it atomically.
- Merge Authorization Requirement: exact current `required` or `not_required` evaluation; missing
  human records never imply automatic eligibility.
- Merge Authorization: one exact single-use Human Owner decision through Admin authority only when
  the current requirement says approval is required.
...
| `packages/project-management/src/adapters/postgres/schema/evidence-quality.ts` | Task evidence, quality review, checks, and reviewers                                           | Import only as historical evidence/review facts. Existing records never satisfy the new Review Gate; current eligibility requires a fresh independent Review over an exact discriminated Snapshot and its variant-required provider, policy/manifest, Docker, decision/package/disposition, containment, and completion facts. |
| `packages/ports/src/issue-tracker.ts`                                          | Current issue-only provider port                                                               | Retire/adapt only behind the Dev Board-facing `DevBoardMirrorPort` facade and focused provider capabilities. Do not expand the legacy port with PR/check/comment/webhook behavior or leak GitHub DTOs into the domain.                                                                                                         |
...
| #229 — Grill and lock the independent Review Gate contract                                                      | **Prepared 2026-07-17; pending reviewed landing, the exact #229 evidence comment and closure, and #228 designation of the exact landed memo plus governing-doc revisions.** [`wf229-review-gate-contract.md`](research/wf229-review-gate-contract.md) proposes independent Reviewer/Attempt authority, adaptive manifests, fresh access, evidence bound to a discriminated `pre_merge` or `provider_result` Review Attempt Snapshot and its variant-required provider facts, exclusive local Docker, rework, authorization, governed merge/reconciliation, and provider-confirmed Done. | —                                              |
...
| #236 — Grill and lock the Releases Gate contract                                                                | **Resolved and closed.** Canonical target contract in `wf236-releases-gate-contract.md` is projected into canonical docs and designated current #237 synthesis input. It defines staging/live separation, build-once manifests, approvals, failure, rollback, evidence, history, and Incident boundaries without claiming implementation.                                                                                                                                                                                                                                               | —                                              |
...
`research/wf231-github-mirror-contract.md` landed at `1db502d722ca33285148251a7660695868ad6a30`;
...
The same three-step lifecycle applies independently to `research/wf229-review-gate-contract.md`:
reviewed landing, the exact #229 evidence comment and #229 closure, then #228 designation of the
exact landed memo plus governing-doc revisions. Before all three complete it is only a prepared
candidate. Afterward it becomes current input for #233, #235, and #237 until #237 consumes and
freezes its Review Attempt, manifest, local-Docker, evidence, rework, merge, provider-result
reconciliation, and Done contract.
...
| #154  | Evidence gate and Lead Orchestrator Quality Review | Quarantine under the prepared #229 candidate. Preserve adversarial/evidence reality goals, but replace Lead-Orchestrator review with fresh independent Review Attempts, deterministic manifests, local Docker, immutable evidence, and authenticated containment. |
| #155  | Human-only Done and merge                          | Quarantine under the prepared #229 candidate: Pass is separate from conditional Human Owner Merge Authorization; governed merge and provider-confirmed exact `development` result gate Done. Staging/production remain Releases.                                  |
...
| Task evidence                                     | Imported historical evidence                                                                                                  | Preserve object/link provenance. It never itself satisfies the new Review Gate; at most, explicitly mapped and authenticated facts may become inputs to a fresh governed Review.                                     |
| Quality review/check/reviewer rows                | Imported legacy Review history                                                                                                | Preserve Review Decision and actors with a `legacy review contract` marker. Do not grant new Done eligibility automatically.                                                                                         |
...
- No code-producing source enters Review without an exact current Deepening binding. Ordinary work
  binds its final checkpoint; an observed provider result binds a completed current Provider Result
  Deepening Capture, authenticated tree, source-read-only worktree containment, governed Docs
  `provider_result` artifact version/hash, and signed capture Runner receipt. A non-code result
  binds its exact current version-bound fact plus explicit Capture/artifact/receipt absence. Legacy
  quality records cannot synthesize either branch.
- No legacy quality row is represented as passing the new Review contract without explicit exact
  reconciliation of Ready; the applicable pre-merge PR/head/base/result/strategy or provider-result
  commit/tree/parents/exact `development` target; independent Reviewer/grants; manifest/policy;
  Docker generation; artifact hashes; containment; current Evidence Disposition; and
  provider-confirmed merge/reconciliation. Provider-result reconciliation additionally rechecks the
  discriminated Deepening applicability branch—current Capture/immutable governed Docs
  `provider_result` artifact/signed Runner receipt IDs/hashes for code-producing work, or exact
  current non-code fact plus explicit Capture/artifact/receipt absence—current Secret-Safe
  Repository Preflight/security-scan pointer generations and accepted IDs/hashes, and exact approved
  Request+Authorization before Done.
...
- Review Handoff tests prove a checkpoint alone never releases implementation capacity; release
  requires a finalized Handoff that atomically emits `ReviewRequested` and allocates/binds the first
  Attempt ID/generation. Fresh Reviewer authority and execution start only afterward. The one
  exclusive shared-Docker lease serializes stack use without blocking unrelated coding. Material
  Revision and Absolute Stop finalizers require the exact contextual **Review Containment Proof**;
  ordinary cutoff and every pre-Running cancellation/non-material supersession first create/reuse
  the exact Review Exit Containment Requirement; Queued/Provisioning with zero activated resources
  persists signed explicit-none resource slots rather than requirement absence. Bounded cleanup
  facts are composed and the Attempt terminalized only by its acceptance finalizer. Successor,
  Review exit, merge, and Done require the resulting distinct **Review Exit Containment Proof**.
  Both proof kinds bind authenticated
  Reviewer/session/worktree-writer/access-grant/Docker/tunnel/artifact-writer closure, and current
  post-package Inspection authority must be absent or contained.
- Review Gate tests prove deterministic decision precedence; trusted non-model pre-push/pre-PR and
  pre-model/output scanning; fresh least-privilege Review Access Grants through the canonical Auth/
  SecretRef path; discriminated pre-merge and provider-result Snapshots; deterministic manifest
  applicability; local-Docker baseline/reset/residue proof; immutable secret-safe packages and
  dispositions; `ReviewChangesRequested` ordering; discriminated Merge Authorization Requirement and
  authorization refusal/expiry; PR ready/draft repair generations and singleton per-PR mutex;
  external-result Capture/prepare/finalize/cancel, Changes Requested or approved persistent-
  Inconclusive remediation Blocked/WIP release, `ResumeExternalResultReview`, and atomic
  Request+Authorization consumption by `ReconcileExternalResultDone`; and provider-confirmed exact
  `development` truth before the applicable discriminated completion command. One fixed real GitHub
  scratch repository proves both required external/manual-result journeys; stubs are supplemental.
  These are real authenticated local-Docker/Postgres/GitHub seams; model output is never the sole
  oracle.
