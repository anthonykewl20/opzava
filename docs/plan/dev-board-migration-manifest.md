...
| `packages/dev-board/` commands `AddDependency`/`RemoveDependency` + `dependencyLockStatus` live read | Dependency graph: workspace advisory graph lock before traversal/mutation; recursive-CTE cycle detection over active edges (`dev_board.dependency_cycle_rejected` — minted, wf230 names none); duplicate-active zero-write no-op; remove classification (`already_removed`/`dependency_not_active`/`dependency_identity_conflict`); retired-edge retention; completion lock = live gate (claim/start reject `dependency_locked` before the TB-02 gate reason; ApproveReadyToTodo deliberately not lock-checked per wf230:1067); todo-lane dependency change atomically invalidates Ready → Backlog. ACCEPTED DEVIATION: `ReadyInvalidated`/`LaneChanged` carried in the `DependencyAdded`/`Removed` activity payload (0020 one-event-per-version uniqueness; full write fidelity) — payload is the authoritative projection source for these consequences. | Landed (TB-01b-4, merged, PR #334) |
| `packages/identity-access/drizzle/0022_dev_board_lane_queue.sql` + `packages/dev-board/` `reorderTodo` + queue store APIs + retrofits | Authoritative Todo lane queue: per-(org,ws,lane) monotonic version headers + sparse bigint rank membership (committed order rank+UUID per wf230:516-517; unversioned positions rejected :2075); before/after/empty-band anchors; midpoint physics (STRIDE 1e6, exhaustion rebalance, overflow guard); CAS queue-version drift (zero writes); ONE `TodoReordered` per command; approveReadyToTodo bottom-append + version declare; dependency todo-exit deletes membership + bumps version in-savepoint; canonical lock order (tickets UUID-sorted → graph advisory → queue). DEFERRABLE one-to-one triggers (SECURITY DEFINER, fail-closed on missing tenant context); `todo_rank` dropped. Consensus-designed; header get-or-create upsert instead of a workspaces trigger (recorded deviation). | Landed (TB-01b-5, merged, PR #336) |
| packages/identity-access/drizzle/0024_dev_board_classifications.sql + packages/dev-board/ classification domain + change-risk policy + SetDevTicketClassification + approveReadyToTodo authorization/completeness/content-binding/policy gates + app.is_active_member/has_organization_role helpers | Classifications (DBF-057-066) + roles gating (DBF-049-056): hybrid storage (nullable-until-Ready CHECK columns + normalized work-area table, classification + policy evaluation mirrored into hashed contract content); versioned deterministic minimum-risk policy (OPA-bundle/CVSS-validated pattern); Backlog-only classification command with planning-ledger materiality rows; Todo → classification_revision_required until the revisions slice; trusted human Ready authorization (kind=user + owner/org-role, never asserted role strings); active-membership gating closes #330's pre-production item; org-scoped ownership decided (closes #330's open question); completeness CHECK NOT VALID + guarded VALIDATE for pre-classification approved rows. | Landed (TB-01b-6, via this PR) |
| packages/identity-access/drizzle/0025_dev_board_archive.sql + packages/dev-board/ archiveDevTicket/restoreDevTicket/restoreProposal + Historical Projection reads | Reversible archive (DBF-178-181): overlay on DevTicket/proposal with lane→backlog resting value + preserved last_active_lane (recorded design decision on #340 — 0021 lane_ready_check/0022 trigger satisfiability), single-statement archive UPDATE clearing all five approval columns, todo-exit in-savepoint, active-dependent (blocker-side) guard, derived human authorization (b-6 wrapper), decisionReason reuse, DevTicketArchived/Restored + ProposalRestored at bumped versions, ArchiveRationaleRecorded planning entries; Historical Projection = derived read-only record_class (archived_dev_ticket/archived_proposal; legacy_historical arrives with import). | Landed (TB-01b-7, via this PR) |
| `packages/identity-access/drizzle/0026_dev_board_legacy_import.sql` + `packages/dev-board/` ImportLegacyDevTicket/ReconcileHistoricalCompletion + legacy alias/historical adapters | Legacy import command surface: source Task rows remain read-only; preserves Task UUID/card-number aliases and immutable source digest/evidence snapshot; imports active records only to Backlog with an explicitly confirmed current Human Owner, quarantines missing-owner rows, and keeps legacy Done historical (`legacy_unverified`) until explicit evidence-bound reconciliation. Reconciliation epoch is retained in immutable planning/activity audit entries; both new tables use tenant RLS. | Landed (TB-01b-8, issue #343) |

GitHub mirror remains deferred to later TB-01 sub-slices / TB-GH1 (see follow-up #330). Legacy import landed in #343 (TB-01b-8).
