...
| `docs/plan/research/wf233-incident-sprint-coordination.md` | Incident/Proposal holds, strict Sprint selection, admission, interruption, and fresh-claim resume      | Resolution candidate for #233 and downstream #237. It reuses WF-230/WF-232/#229 owners, never persists Incident as Sprint work, and makes no implementation claim.                                         |
...
- Sprint Coordination: Plan-bound serial selection, holds/waits, and Sprint entitlement; never a
  DevTicket claim, Incident lifecycle, or generic dispatcher.
- Coordination Interruption Request: redacted exact target-set request whose independent targets
  reuse their WF-230/#229 containment owners.
...
| #233 — Reconcile Incident projections, governed interruption, strict Sprint order, and ordinary-work preemption | **Resolution candidate.** [`wf233-incident-sprint-coordination.md`](research/wf233-incident-sprint-coordination.md) specifies per-Runner entitlement/activation barrier, strict selection and no scan-ahead, global Review-WIP consumption, explicit ordinary claims, Incident/Proposal holds, phase-safe WF-230 containment, multi-target partial state, no-failover reconciliation, changes-requested races, explicit Resume/fresh claim, and deterministic real-seam evidence. It becomes current #237 input only after verified landing, closure, and map designation. | #230, #232                                     |
...
`research/wf233-incident-sprint-coordination.md` is the reviewed resolution candidate for #233. It
does not become current merely from this link: after its exact commit is landed, #233 is closed, and
map #228 explicitly designates it current input, #237 must consume and freeze its strict selector,
Incident/Proposal hold, Review-WIP, phase-safe interruption, explicit-resume, and deterministic
evidence constraints. Until that tracker/map sequence completes, it is target candidate evidence,
not executable implementation authority.
...
- Sprint coordination tests pass for the versioned activation barrier, strict blocking→Plan→bottom-
  rework selector and no scan-ahead, dependency wait through Done, WIP-three/gate-open fairness,
  blocking-Proposal and Incident holds, pause-versus-claim/start/Review arbitration, multi-target
  partial containment, `checkpoint_not_recorded`, changes-requested versus later start, and explicit
  Resume/fresh claim. No Incident identity may appear in DevTicket Type, Sprint membership, or the
  GitHub Sprint mirror.
...
- One real authenticated browser vertical story passes on the real local Docker stack, including
  Balanced Sprint-plus-ordinary work, Review-WIP backpressure, version-bound Incident interruption
  approval, proof-gated Todo, blocking Proposal/Plan revision, changes-requested race, Runner
  disconnect/no failover, explicit Resume/fresh claim, and immutable Sprint completion while the
  Incident lifecycle stays separate.
