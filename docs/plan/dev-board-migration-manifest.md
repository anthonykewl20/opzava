...
| #235 — Lock archive, retention, exceptional redaction, tombstone, and revocation behavior                       | **Resolution candidate, not yet closed/current.** [`wf235-archive-retention-redaction-revocation.md`](research/wf235-archive-retention-redaction-revocation.md) proposes the record taxonomy, prospective policy binding, archive/restore ownership, exceptional-redaction authorization, content suppression, per-target truth, evidence-reliance revocation, provider/backup seams, and deterministic validation for later verified resolution and #237 consumption. No product behavior is claimed.                              | #230, #231, #232                               |
...
`research/wf235-archive-retention-redaction-revocation.md` is a #235 resolution-candidate asset, not
current input merely because it is linked here. After #235 is independently verified and closed,
#228 or this manifest must explicitly designate the exact landed revision current for #237; #237
then consumes/freezes it. Until that sequence, its proposed `DevBoard.RetentionPolicy` records,
archive/restore rules, per-target dispositions, suppression/tombstone/reliance semantics, and
provider/backup validation authorize no implementation issue.
...
| Legacy archive flags, closed projections, or hidden rows          | Owner-specific archive overlay plus Historical Projection                                                                       | Classify source/owner/provider fact exactly. V1 non-Done archive adds the safe marker, removes lane status, and closes with `state_reason=not_planned`; Done adds the marker while preserving `status:done`/completed close. Non-Done restore reopens to `status:backlog`; Done restore preserves closed-completed. Never revive prior authority or redacted bytes. |
| Existing retention/TTL configuration and timestamps               | Candidate `RetentionPolicyVersion` plus immutable per-record ingestion binding                                                  | Do not backdate or shorten policy to fit current age. Ambiguous/unbound records remain quarantined/durable until a prospective binding and authorized disposition exist.                                                                                                                                                                                            |
| Raw logs, telemetry, previews, and transient artifacts            | Exhaustive durable/no-TTL, bounded-raw, or never-persist classification plus explicit pins/holds                                | Never import suspected-secret or unredacted raw content. Bounded raw requires a durable safe summary, adapter capability, replay/audit window, and truthful per-target expiry semantics.                                                                                                                                                                            |
| Prior deletes, redactions, secret incidents, and provider cleanup | `ExceptionalRedactionCase`, per-target observations, `AuditTombstone`, and owner-directed revocation legs where evidence exists | Never invent erasure proof, reconstruct removed bytes, or collapse local cleanup into upstream revocation. Unknown/manual/unsupported/residual targets remain visible and quarantined.                                                                                                                                                                              |
| Evidence/artifact rows with missing or removed payload            | Historical fact plus `EvidenceRelianceRevoked` when a current binding relied on it                                              | Do not resurrect bytes from backup, mirror, cache, or rebuild. Preserve Done/Released history with assurance-compromised and Incident/follow-up linkage; active bindings fail closed.                                                                                                                                                                               |
...
### Cross-ledger retention and exceptional-removal migration

- Keep retention bindings, holds, manifests, cases, suppression barriers, tombstones, and reliance-
  revocation facts in/cross-linked to their four owning ledgers; create no fifth ledger or fictional
  global sequence.
- Backfill only facts that the source can prove. Every imported record receives an explicit
  durable/no-TTL, bounded-raw, never-persist (safe metadata-only quarantine), archived, or
  historical disposition plus source and policy refs; absence is not deletion proof. Suspected-
  secret raw bytes are rejected/disposed, never quarantined.
- Establish the `PayloadSuppressionBarrier` and independently replicated erasure stream before any
  legacy raw store, search index, export, mirror, or backup can become a target read source.
- Keep owner revocation legs separate: GitHub Integration, Runner/Execution Admission, secret
  broker, Review/preview, artifact/publication, and Incident owners retain their lifecycle
  authority. Migration/retention may request and observe but cannot finalize those states.
...
2. **Inventory and backup.** Count every Task, child row, evidence/review row, issue projection,
   create intent, outbox row, runtime outcome ref, route deep link, relevant GitHub issue/comment,
   raw/derived copy, archive flag, retention setting, legal/secret removal, revocation leg, search/
   export copy, and backup generation. Capture a reproducible migration report plus an independently
   replicated erasure/suppression stream before schema changes.
3. **Add target storage and commands.** Create the dedicated Dev Board context, RLS-protected
   ...
   runner protocol, and `DevBoard.RetentionPolicy` records/ports without redirecting current routes
   or granting retention any owning-domain lifecycle authority.
4. **Backfill deterministically.** Preserve IDs and aliases, apply the mapping table above, attach
   source refs, and quarantine every ambiguous record using safe metadata only. Reject/dispose
   suspected-secret raw bytes before domain ingress. Bind retention prospectively, preserve existing
   archive/history, record tombstones without raw copies, and never resurrect suppressed bytes. Do
   not silently promote Ready, assign roles, or bless legacy evidence.
...
- Secret exposure and unverifiable-GitHub bypass attempts fail through UI, Slack, GitHub, agent, and
  command seams.
- Every imported record is classified durable/no-TTL, bounded raw, or never-persist with only safe
  metadata quarantine and has an immutable prospective policy binding or an explicit reason it
  cannot expire. Suspected-secret raw bytes are absent. No cutover script shortens/backdates
  retention or treats archive as TTL/delete/Done.
- Archive/restore tests prove owner-specific commands; non-Done safe marker, active-lane removal,
  WF-231 outbox close with `state_reason=not_planned`; Done marker with preserved `status:done` and
  completed close; non-Done marker removal, reopen, and `status:backlog`; Done marker removal with
  Done/closed-completed preserved; mirror-pending/unknown reconciliation and provider-gate wait; and
  no resurrection of Ready approval, assignment, claim, lease, Sprint membership, Runner authority,
  or redacted bytes.
- Exact-CAS expiry blocks on pins, holds, reliance, replay/gaps, provider unknown, and concurrent
  record/policy changes. Every store/provider exposes truthful success/not-found-with-proof/unknown/
  manual/unsupported/residual outcomes; no partial result is reported as global erasure or blind-
  retried.
- Secret-Safe Ingress and `PayloadSuppressionBarrier` canary tests prove suspected-secret/exact
  suppressed bytes are absent or unreadable across domain rows, logs, queues, mirrors, GitHub,
  evidence, search, exports, backups, restore/re-ingest/rebuild, and model seams while safe workflow
  facts remain.
- Exceptional redaction proves automatic exact-minimum active-exposure containment, Admin step-up
  for broader/legal/destructive action, one-use target authorization, safe keyed tombstones, visible
  residual/manual/unsupported state, and separate local versus upstream owner revocation legs.
- Evidence-reliance removal invalidates active owner bindings without rewriting historical Done/
  Released facts and preserves assurance-compromised plus Incident/follow-up evidence. Backup
  restore stays isolated until the independent erasure stream replays without gaps or unknown
  targets.
...
- archive, retention, exceptional redaction, tombstone, reliance revocation, provider truth, and
  backup restore have explicit classifications, owners, authorizations, per-target dispositions,
  no-resurrection rules, and independently verified cutover evidence;
