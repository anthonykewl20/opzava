...
| `glm-exec` | Delegate artifact generation to GLM 5.2 (z.AI) via the user's `claude-glm` setup (stock claude CLI, separate `~/.claude-glm` config dir), same wrapper+digest contract as `codex-exec`, so slices can be assigned to Claude/GPT/GLM interchangeably under the same gated workflow + real-world-validation verdict. | Now (multi-executor orchestration) | [x] created + LIVE-verified (doctor READY, real z.AI ping) — `~/.claude/skills/glm-exec/` + `~/.claude/scripts/glm-exec.sh` |
