...
| Field | Value |
| --- | --- |
| Active slice | Slice 3.7 - PORT OpenClaw Control UI -> Opzava admin dashboard (program spec: `docs/plan/consensus/port-openclaw-control-ui-program.md`, amended by Q18; folds in 3.6 standardization) -> THEN Dokploy VPS bring-up (gated on user buying `opzava.app`). Mainframe-move slice (Q18/ADR-016, issue #126) DONE 2026-07-04: gateway builds+runs from `./mainframe`, real-world-validation gate DONE-ELIGIBLE. |
...
| Next concrete action | Slice 3.7 view #1 (Models & Providers) DONE (2026-07-04): grouped/curated LLM catalog, runtimes-under-parent, real models (`models.list`) + 5-state status (`models.authStatus` RPC primary, CLI fallback), gateway-gated canonical connect targets (has-zai/has-openrouter live), real onboard-exec connect through the worker; `real-world-validation` gate DONE-ELIGIBLE exit 0 (2 clean passes). NEXT = view #2 Overview (`overview.ts` + overview-cards/attention/event-log/hints/log-tail; worker/broker reads): reconcile a mockup (author if missing) against the OpenClaw reference, implement through the ACL, prove via real login + logs + the gate. Then views #3 Agents, #4 Sessions, ... per the program table. Dokploy bring-up checklist (later, on user go): buy `opzava.app`, wildcard A -> 5.189.186.18, create Dokploy project from the repo (builds all images on-box; pre-agreed CI->ghcr fallback for the mainframe image), Traefik+LE on 443 + local mkcert TLS parity (compose currently has NO profiles key - see ARCHITECTURE.md deployment topology), verify wss live, rotate the Dokploy API key + panel TLS. |
