...
| Next concrete action | Slice 3.7 view #1 (Models & Providers) under the Q18 contracts: (1) reconcile `ux-redesign/mockups/connections.html` against the OpenClaw reference (`mainframe/ui/src/ui/views/config-form.*`, `model-auth-helpers`, `chat-model-*`, `gateway-methods.ts`) - mockup-revision-first; (2) implement per the program spec first-slice section (canonical providers per `docs/openclaw/providers`, runtimes UNDER parent, z.AI/OpenRouter, real models list/status, real onboard-exec connect through the worker); (3) prove: real-login browser test + snapshot booleans + worker/gateway logs + `real-world-validation` gate exit 0 (run BARE, never piped). Dokploy bring-up checklist (later, on user go): buy `opzava.app`, wildcard A -> 5.189.186.18, create Dokploy project from the repo (builds all images on-box; pre-agreed CI->ghcr fallback for the mainframe image), Traefik+LE on 443 + local mkcert TLS parity (compose currently has NO profiles key - see ARCHITECTURE.md deployment topology), verify wss live, rotate the Dokploy API key + panel TLS. |
