...
- [ ] GPT Pro / Codex CONNECTION GUI (user directive 2026-07-03): on the SAME thin Connections surface, a model-provider card matching `connections.html`'s "Sign in with ChatGPT (codex OAuth)". "Connect" triggers the platform Gateway's OpenAI device-code flow through the PROVISIONING/admin path (JIT `operator.admin`, NOT the broker hot path per Q16/ADR-003) and surfaces the device URL + code in the browser (same flow the CLI runbook does), polling to connected state; shows connected account, `auth.order.openai` (OAuth -> API-key fallback), the "running on API-key fallback" monitoring state, and reconnect/disconnect. Tokens stay in the Gateway's auth-profiles volume (never leave that environment, never in the browser) - the GUI only orchestrates the connect, it never handles token material. Replaces the manual in-container `onboard` runbook step for the common case; runbook retained as fallback.
