...
- 2026-07-12 (connections actions polish) - CONNECTED-PROVIDER ROW ACTIONS POLISHED: connected
  model-provider rows now use a shadcn/Radix row-actions menu
  (`aria-label="Row actions for <provider>"`) for Manage, eligible Set as main orchestrator, and
  Disconnect, the non-actionable "Actions" menu label is intentionally absent, the lead keeps a
  visible Main orchestrator badge, and available providers keep the inline Connect button.
- 2026-07-12 (orchestrator) - MAIN-ORCHESTRATOR SELECTION (issue #129) BUILT +
  ...
  realtime auto-refresh stays out of scope (#128).
