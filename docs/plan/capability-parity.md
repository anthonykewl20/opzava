...
| `connections.html` - gateway/models, provider auth, agent tools/MCP, channels/services | Hybrid | `health`, `models.list`, `usage.status`, `channels.status`, `web.login.start/wait`, `tools.catalog/effective`, `skills.status`, `config.schema`. | External Channels via ACL | L |
