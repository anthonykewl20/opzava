...
Opzava needs a complete tenant-facing billing and entitlement surface before external monetization; until then, `BillingPort` ships as a null adapter with local entitlements only.

Without this PRD, the billing slice can drift into unsafe or confusing product behavior:

- Owners may later see a provider subscription but not understand which Opzava capabilities the plan actually allows.
...
The retained future solution is a Billing settings and onboarding-completion product slice owned by Finance and Billing, with Stripe isolated behind `BillingPort` when external monetization starts, plan and quota decisions exposed in Opzava language, `usage.cost` converted into durable metering/read models, and tenant dunning/suspension state surfaced wherever it affects access. This PRD applies ADR-014 and ADR-002, depends on PRD-001 and PRD-013, and references the existing mockups without restating architecture.
...
- When external monetization starts, route Manage plan, payment method updates, cancellation, renewal, tax/address, invoice download, and plan changes through Stripe Billing Portal or provider-specific checkout sessions behind `BillingPort`.
...
- Build the payment provider itself, replace provider invoices, or expose provider object ids as product concepts.
...
- Let a payment provider become the source of truth for Opzava product entitlements, tenant lifecycle, plan limits, or Gateway runtime state.
...
6. As an owner, I want payment method changes to happen through provider-hosted flows, so that card and bank details never enter Opzava forms.
...
25. As an owner, I want overage responses to identify the exceeded Opzava limit, so that I do not see provider internals or raw quota codes.
...
46. As a billing admin, I want future provider metered usage posting failures to show safe status and retry state, so that support can repair without data loss.
...
71. As a security reviewer, I want provider webhook payloads verified and stored only as safe refs/audit metadata, so that provider payloads do not leak sensitive data.
...
76. As a developer, I want tests at application-service, port, quota middleware, webhook, projection, and UI seams, so that billing behavior is protected without coupling to payment-provider or OpenClaw internals.
...
| --- | --- |
| `settings.html` | Settings page keeps Billing as a first-class left-nav panel. The Billing panel must show current plan badge, entitlement/dunning state, monthly spend, budget used, seats used/allowed, monthly budget meter, monthly budget input, billing email input, payment method summary, alert threshold selector, Pause new runs at budget cap switch, monthly receipts switch, recent invoice table, Save billing settings, Manage plan, and renewal/trial/grace date. When external monetization starts, Manage plan opens a server-created Stripe Billing Portal session through `BillingPort`. The Billing panel must add plan-limit visibility for agents, channels, autonomy tiers, workflow mechanisms, provider/model access, and runtime/budget policy, either inline or via linked plan detail drawer. |
...
- Plan comparison and checkout/upgrade entry screen before handing off to provider-hosted checkout or billing portal when external monetization starts.
...
- Usage event or invoice-detail drawer for line-item explanation, meter receipts, adjustments, future provider posting state, and reconciliation status.
...
- Until external monetization starts, `BillingPort` uses a null adapter with local entitlements only. When Stripe is added, it must be accessed only through `BillingPort`; product code, route handlers, workers, provisioning, and UI must not call Stripe SDKs directly.
- Stripe customer, subscription, invoice, payment, price, portal-session, checkout-session, usage-record, and webhook ids are future adapter refs and must remain opaque external refs.
...
- When external monetization starts, Manage plan must create a server-side provider portal session for the authorized tenant and active billing customer.
...
- `Plan` records must represent Opzava product limits in `limits_json` or equivalent structured policy, not only future provider price metadata.
...
- Over-limit responses must name the Opzava limit, current usage, allowed limit where safe, affected action, and next action. They must not leak payment-provider internals.
...
- `UsageMeter` must aggregate by tenant, period, agent, department, channel, workflow, mechanism, feature, provider/model where known, cost class, and future provider meter dimension where required.
- Usage dashboards must show billing-period totals, budget status, spend by dimension, meter freshness, last poll/checkpoint, adjustment status, and future provider posting state.
...
- Metering failures, usage spikes, future provider posting failures, or reconciliation deadletters may create PRD-012 incidents, but incidents are not the source of truth.

### Dunning, suspension, and tenant lifecycle surfacing

- When external monetization starts, provider webhooks and `BillingPort` reconciliation must update `Subscription` and `Invoice` dunning state.
...
- Setup must be idempotent by tenant/setup/provisioning identifiers and must not create duplicate future provider customers/subscriptions, tenants, owners, or Gateway instances on retry.
...
- Onboarding progress must show user-safe step names and statuses without exposing Gateway paths, ports, tokens, provider refs, or secrets.
...
| Provider portal/session handoff | Finance and Billing | Authorized tenant/customer ref, portal session ref, return url, provider result, subscription reconciliation | `BillingPort`, `AuthorizationPort`, `EventBusPort` |
...
| Meter receipts | Finance and Billing | `MeterEvent`, idempotency key, raw payload hash, quantity, cost, accepted/corrected state, future provider posting state | `BillingPort`, `EventBusPort` |
...
| --- | --- |
| `usage.cost` observations | OpenClaw-native runtime source harnessed through the broker ACL. Opzava owns immutable `MeterEvent` receipts, `UsageMeter` projections, dashboards, future provider usage posting, corrections, and invoice explanations. |
...
| Stripe/payment provider | Not OpenClaw-native. No provider is implemented until external monetization; when added, Stripe is isolated behind `BillingPort`. Opzava owns commercial policy, entitlements, and product state. |
...
- Until external monetization starts, keep `BillingPort` as a null adapter with local entitlements only. When Stripe is added, keep it behind `BillingPort`; use hosted portal/checkout flows for payment method, invoice, tax/address, cancellation, renewal, and plan-management actions.
...
- Keep plan policy structured and versioned so subscription changes, temporary entitlements, and future provider replacement do not require provider-specific product logic.
...
- When external monetization starts, Manage plan creates a provider portal session only for the authorized tenant and does not expose raw provider customer, subscription, payment, or secret values to the browser.
...
- Setup retry does not duplicate tenant, owner, future provider subscription/customer, provisioning job, Gateway instance, or invites.
...
- Use fake `BillingPort`, fake `OpenClawGatewayPort`, fake `GatewayRuntimePort`, and tenant-scoped repositories; do not test against live payment providers or live OpenClaw.
...
- Metering tests must cover idempotent duplicate ingestion, late windows, replay after checkpoint, changed raw payload correction, future provider posting retry, posting success, posting failure, and read-model rebuild.
...
- Tests must assert no raw provider ids, payment details, Gateway tokens, channel credentials, provider secrets, or raw `usage.cost` payloads appear in ordinary UI responses, logs, or exported billing files.
...
- ADR-013 incident pipeline: metering failures, dunning failures, usage spikes, stale projections, and future billing/provider posting failures as incidents where appropriate.
