# PRD-003: Projects, boards, cards, goals, to-dos, schedules, docs, and discovery

> **Scope amendment (2026-07-15):** PRD-003 continues to own generic Project Management, including Project, board, `pm.Card`, to-do, goal, schedule, docs links, and Discovery. It no longer owns the Opzava platform-development Tasks/Issues surfaces or GitHub issue execution workflow. PRD-019 owns Dev Board and its `DevTicket`; `DevTicket`, `pm.Card`, and OpenClaw `workboard.Card` are three different concepts. The old full/admin Tasks and Issues mockup requirements below are superseded wherever they conflict with PRD-019, ADR-017, or `docs/plan/dev-board-migration-manifest.md`.
...
The mockups show a broad project workspace: projects have tools, boards, cards, to-dos, goals, schedules, docs/files, and discovery. A card can be ordinary human work, AI-assisted work, customer-facing work, or a bridge into agent execution. Users need the same work to be understandable as a project card, a board row, a calendar item, a personal to-do, a goal contributor, and an AI run summary.
...
- AI work could disappear into runtime tasks instead of projecting back into the human card, goal, to-do, schedule, notification, or doc where the user expects to review it.

The solution is an Opzava-owned Project Management experience that uses ADR-004 read/write boundaries, ADR-007 authorization, ADR-008 `AgentDispatch`, and ADR-010 project knowledge corpora. OpenClaw runtime work is harnessed through the broker and projected back, but project identity, `pm.Card`, boards, goals, to-dos, events, docs metadata, and discovery records remain Opzava-owned.
...
- Make Project the main child scope for boards, cards, goals, to-dos, schedules, docs/files, discovery, comments, activity, and project knowledge.
...
- Build or own Dev Board, `DevTicket`, GitHub issue synchronization, Sprint execution, code review, development delivery, or releases. Those belong to PRD-019 and ADR-017.
...
65. As a project manager, I want an optional DevTicket link on relevant project work, so that product planning and platform implementation can be cross-referenced without merging their lifecycles.
66. As a project member, I want a linked DevTicket summary to open Dev Board, so that implementation status is visible at its source of truth.
67. As a project manager, I want creating generic `pm.Card` work to remain independent of Dev Board readiness and Sprint gates, so that ordinary project management is not forced into the engineering workflow.
68. As a project manager, I want Dev Board execution to use its own assignment and dispatch rules, so that `pm.Card` is never silently promoted into a DevTicket.
...
71. As a project member, I want unauthorized cards, docs, linked DevTicket summaries, and schedule items to be hidden or forbidden consistently, so that private work does not leak.
...
| `task-board.html` | **Superseded target.** This historical full/admin Tasks mockup may inform migration only. PRD-019 defines the target Dev Board; it does not redefine the generic Project Management `pm.Card` board. |
...
| `issues.html` | **Superseded target.** This historical standalone Issues mockup is a migration input only. PRD-019 owns GitHub-backed Dev Board work; PRD-012 owns the separate Incident aggregate and lifecycle. |
...
- Project archive must preserve cards, goals, to-dos, schedules, docs metadata, discovery records, linked-work refs, activity, and dispatch history.
...
- Card creation must support title, description, source, labels, due date, assignee, column/status, steps, linked goal/to-do/docs, and optional external-work ref.
...
- Archive card must preserve audit, comments, evidence metadata, dispatch refs, and linked-work refs.
...
- Connected work may include to-dos, cards, boards, docs, schedule events, discovery records, and linked-work refs where authorized.
...
- Project Management may attach docs/files/evidence refs to projects, cards, to-dos, goals, discovery records, and linked work.
...
- Platform-development work is a Dev Board `DevTicket`, not a Project Management `pm.Card` and not an OpenClaw `workboard.Card`.
- Project Management may hold an opaque link to a DevTicket when product work needs cross-reference, but it does not own DevTicket lifecycle, GitHub synchronization, Sprint membership, readiness gates, review evidence, or merge/release state.
- GitHub authority is split by fact: GitHub owns the repository-native issue number/URL and PR, commit, check, and merge facts; Opzava owns Dev Board workflow, gates, approvals, plans, and execution policy. Shared content synchronizes deterministically under ADR-017.

### Activity, updates, realtime, and states

- Project Management must emit events for project, card, goal, to-do, schedule, discovery, docs-link, and linked-work changes.
...
| Dev Board link | Dev Board, with optional Project Management reference | Opaque `DevTicket` ref and display summary only; lifecycle, GitHub facts, sync, Sprints, and review remain Dev Board-owned | `AuthorizationPort`; future Dev Board application port per PRD-019 |
| Project activity and updates | Project Management with contributing contexts | Activity entries for project/card/goal/to-do/schedule/docs/discovery/linked-work/dispatch events, latest activity read models | `EventBusPort`, `AuthorizationPort`, `RealtimeTransportPort` |
...
| Project tools/cards | Opzava-owned | Goals, to-dos, board, schedule, docs links, discovery, updates, and linked-work refs are Opzava project configuration and aggregates. |
...
| Historical full/admin Tasks board | Superseded migration input | The current `/tasks` implementation remains real code until migrated, but PRD-019 Dev Board is the future source for platform-development execution. It never changes the identity or lifecycle of generic `pm.Card`. |
...
| GitHub-backed development work | Hybrid, Dev Board-owned | PRD-019/ADR-017 own the one-repository Dev Board workflow and deterministic GitHub synchronization. PRD-003 may retain only opaque cross-links. |
...
- Runtime projection loss or Gateway downtime degrades AI status without deleting or corrupting the owning card/to-do/goal/event/work record.
...
- Docs/files metadata can attach to projects/cards/to-dos/goals/discovery/linked work and can be promoted into project knowledge only through ADR-010 flows.
...
- Optional DevTicket links expose only authorized summaries and route to Dev Board; they do not duplicate Dev Board workflow controls inside Project Management.
- Creating or dispatching a `pm.Card` never creates a DevTicket implicitly.
...
- DevTicket-link tests must prove the reference is optional, authorization-filtered, and never treated as `pm.Card` identity or lifecycle authority.
- RBAC tests must cover Owner/Admin/Manager/Member/Guest-Client differences at project, card, docs, schedule, discovery, and DevTicket-link visibility.
...
- External calendar/content and approved linked-work integrations may be stubbed behind provider ports for first implementation, but the UI must show explicit sync/unavailable states when providers are not connected.
