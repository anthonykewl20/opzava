...
23. As an owner, I want provider routing policy to show and change the single main orchestrator provider and show which providers back specialist workers, so that cost and capability choices are visible.
...
- Provider inventory must show provider, auth method, configured models, status, last check, and policy/routing state.
...
- Provider routing must support exactly one main orchestrator, subagent specialists, content workers, research workers, local CLI workers, and future model classes without hardcoding provider-specific behavior into product code.
- The main orchestrator is the connected provider whose model is the Gateway primary model at `agents.defaults.model.primary`.
- Changing the main orchestrator must require a connected provider with a routable model, patch `agents.defaults.model.primary` through the admin-scoped provisioning worker, and rebuild subagent delegation so every other connected provider remains available as a subagent.
...
- Provider/model table shows provider, auth, configured models, status, connected tier counts, exactly one main-orchestrator indicator, and a Set as main orchestrator action for connected non-leads.
...
- Provider/model tests must cover catalog availability, default model validation, auth order, SecretRef health, near-limit/quota-exceeded states, ADR-014 entitlement denial, single-main-orchestrator selection, subagent rebuilds, routing policy changes, and audit.
