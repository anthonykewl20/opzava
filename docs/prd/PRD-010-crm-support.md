...
25. As a support lead, I want SLA breach and warning states to feed the Ticket queue, AI assignment/workload projections, Activity, notifications, and dashboards, so that time-sensitive support work is not missed.
...
44. As a support lead, I want a Ticket board/table view to use CRM-owned support statuses consistently, so that support workflow stays familiar without inheriting development-work lanes.
45. As a support lead, I want AI assignment/workload rows for support work to deep-link to the owning Ticket, Contact, and approval, so that operational visibility does not fork workflow truth.
...
| `task-board.html` | **Superseded visual reference.** The historical admin Tasks board does not define future CRM workflow. Net-new user-dashboard design is required for a Support Ticket Queue/Board with Contact, Account, channel, SLA, priority, consent, UnknownContact, reply-ready state, and AI-assignment projections; the CRM Ticket remains its source of truth and is not a DevTicket. |
...
- Prior-art seams to follow are PRD-003 project/card tests, PRD-004 collaboration/activity tests, PRD-006 AI Workforce assignment/workload and run-evidence tests, PRD-007 Knowledge scrub/rebuild tests, and PRD-009 consent/report handoff patterns.
...
- PRD-006 for AI assignment/workload projections that deep-link to the owning Ticket or `pm.Card`, run evidence, Support employee operations, and tool/catalog health.
