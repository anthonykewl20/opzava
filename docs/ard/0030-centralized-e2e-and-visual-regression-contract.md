...
5. **Determinism recipe:** chromium-only · locale `en` · animations off (`prefers-reduced-motion` + style kill) · gateway-off · mask non-seed regions; a fixed admin seed plus storageState auth via Playwright `globalSetup` against the fresh-per-run DB.
...
- **Authenticated VR is storageState-based.** Per-test UI login does not scale because it trips the login rate limiter at 16+ specs. The pattern is one API login in `globalSetup` (`POST /api/auth/login` with retry/backoff), save `tests/visual/.auth/user.json` (gitignored), then restore that storage state in a fresh browser context per `authPage` test.
- **Screenshot tolerance is centralized and tight.** Visual specs use `VISUAL_MAX_DIFF_PIXEL_RATIO = 0.01` from `tests/visual/fixtures.ts`, tighter than Playwright's `0.2` default; the deterministic harness and app-chrome masks have been stress-tested at that threshold.
- **`E2E_USE_CHROME=1` is a local-only escape.** It lets local machines use system Google Chrome when bundled chromium is unavailable; CI uses Playwright's bundled chromium so baselines stay on one browser substrate.
