...
A user's **local** agent tool (Claude Code/Desktop, Codex CLI/Desktop, opencode — the **five local clients**) must authenticate to Opzava running on a **cloud Dokploy** instance. **(Amended: OpenClaw and Hermes are SERVER-ONLY runtimes co-located with Opzava on the deploy host — detected by-file, never device-authenticated from a laptop — so device-auth is out of scope for them; see `gateway-setup.md` / CONTEXT.md two-plane split.)** Today the only options are a static global `API_KEY` (`auth.ts:487-502`, unconditional `role:'admin'` at `:495`) or a profile-file/cookie copied by hand — no per-device identity, no rotation, no revocation, and the credentials persist as cleartext at default file mode (0644). This does not meet "real-world reliability hardening" for the local-agent ↔ cloud link.
