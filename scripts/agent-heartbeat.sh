...
# Opzava Phase 3: Agent Heartbeat Script
...
# Check if Opzava is running
check_mission_control() {
    if ! curl -s "$MISSION_CONTROL_URL/api/status" > /dev/null 2>&1; then
        log "ERROR" "Opzava not accessible at $MISSION_CONTROL_URL"
        return 1
    fi
    return 0
}
...
send_wake_notification() {
    ...
    # Format wake message
    local wake_message="🤖 **Opzava Heartbeat**\n\n"
    ...
    wake_message+="Use Opzava to view details: $MISSION_CONTROL_URL\n\n"
...
main() {
    ...
    if ! check_mission_control; then
        log "ERROR" "Aborting: Opzava not accessible"
        exit 1
    fi
...
case "${1:-}" in
    --help|-h)
        echo "Opzava Agent Heartbeat Script"
        echo ""
