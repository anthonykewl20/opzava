...
type PluginManifestProviderAuthChoice = {
  ...
  choiceId: string;
  /** Profile ids written together by this auth choice, including cross-provider siblings. */
...
function normalizeProviderAuthChoices(
  ...
  for (const entry of value) {
    ...
    const ownedProfileIds = normalizeTrimmedStringList(entry.ownedProfileIds);
    ...
    normalized.push({
      ...
      method,
      choiceId,
      ...(ownedProfileIds.length > 0 ? { ownedProfileIds } : {}),
