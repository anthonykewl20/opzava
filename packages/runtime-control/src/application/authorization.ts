...
const runtimeControlRoles = new Set(["owner", "admin", "member"]);
...
function roleAllows(required: TenantGrant, roleKeys: readonly string[]): boolean {
  ...
  if (required === "member") {
    return roleKeys.some((roleKey) => runtimeControlRoles.has(roleKey));
  }
...
export class RoleKeyRuntimeControlAuthorizationPort implements AuthorizationPort {
  public async can(
    subject: AuthorizationSubject,
    action: AuthorizationAction,
    resource: AuthorizationResource
  ): Promise<Result<AuthorizationDecision>> {
    if (resource.type !== "agent" && resource.type !== "workspace") {
    ...
    return roleDecision(
      roleAllows("member", subject.roleKeys ?? []),
      "missing-runtime-control-role"
    );
  }
...
export const defaultRuntimeControlAuthorizationPort =
  new RoleKeyRuntimeControlAuthorizationPort();
