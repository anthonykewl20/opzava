...
const taskManagerRoles = new Set(["owner", "admin", "member"]);
...
function roleAllows(required: TenantGrant, roleKeys: readonly string[]): boolean {
  ...
  if (required === "member") {
    return roleKeys.some((roleKey) => taskManagerRoles.has(roleKey));
  }
...
export class RoleKeyTaskAuthorizationPort implements AuthorizationPort {
  public async can(
    subject: AuthorizationSubject,
    action: AuthorizationAction,
    resource: AuthorizationResource
  ): Promise<Result<AuthorizationDecision>> {
    if (resource.type !== "task" && resource.type !== "workspace") {
    ...
    return roleDecision(roleAllows("member", subject.roleKeys ?? []), "missing-task-role");
  }
...
export const defaultTaskAuthorizationPort = new RoleKeyTaskAuthorizationPort();
