...
export interface MemorySearchProviderReconcilerOptions {
...
export interface MemorySearchProviderReconcileReceipt {
  /** Whether the gateway accepted the memory-search provider opt-out patch. */
...
export class MemorySearchProviderReconciler {
  private readonly maxAttempts: number;
  private readonly retryBackoffMs: readonly number[];
  private readonly sleep: (ms: number) => Promise<void>;

  public constructor(private readonly options: MemorySearchProviderReconcilerOptions) {
  ...
  public async reconcile(): Promise<Result<MemorySearchProviderReconcileReceipt>> {
    const patched = await this.retry(async () => {
      ...
      const patchParams = configPatchParams({
        ...
        patch: { agents: { defaults: { memorySearch: { provider: "none" } } } },
      });
      if (!patchParams.ok) {
        return patchParams;
      }
    ...
    if (!patched.ok) {
      // Unlike the security-critical SecretRef reconcile, this only suppresses a cosmetic doctor
      // warning, so failure must not block worker startup.
      console.warn("workers.memorySearchProvider.reconcileFailed", patched.error);
