...
function reconciler(adminClient: GatewayAdminClient, maxAttempts = 5) {
  return new PhoneControlDisableReconciler({
    adminClient,
    maxAttempts,
...
describe("PhoneControlDisableReconciler", () => {
  it("sends one atomic phone-control disable patch with operator.admin scope", async () => {
