...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  if (
    ...
    body.requestId.trim() === "" ||
    body.requestId.trim() !== body.requestId ||
    body.requestId.length > 128 ||
    typeof body.providerId !== "string" ||
    body.providerId.trim() === ""
  ) {
    return NextResponse.json(
      { message: "A valid requestId and providerId are required", code: "web.invalidRequest" },
      { status: 400 },
    );
  }
  ...
  const result = await setMainOrchestratorForContext(
    context,
    body.requestId,
    body.providerId,
    model,
