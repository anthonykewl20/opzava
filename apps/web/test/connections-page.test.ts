...
describe("Connections page state", () => {
  ...
  it("wires /connections routes, actions, API poll route, and sidebar without JSX imports", async () => {
    ...
    const systemPage = await readRepoFile("app/(app)/connections/system/page.tsx");
    ...
    const systemLoading = await readRepoFile("app/(app)/connections/system/loading.tsx");
    ...
    expect(systemPage).toContain("ConnectionsSystemStatus");
    expect(systemPage).toContain("ConnectionsAutoRefresh");
    expect(systemPage).toContain("PageNotice");
    expect(systemPage).toContain("noticeFromSearchParams");
    expect(systemPage).toContain('refreshConnectionsAction.bind(null, "/connections/system")');
    expect(systemStatus).toContain("openclawHealthSummary(health)");
    expect(systemStatus).toContain("System Core");
    expect(systemStatus).toContain("Channels");
    expect(systemStatus).toContain("Agents");
    expect(systemStatus).toContain("Sessions");
    expect(systemStatus).toContain("Gateway detail");
    expect(systemStatus).toContain("Runtime");
    expect(systemStatus).not.toMatch(/session key|raw path|prompt/i);
    expect(accordion).toContain("@radix-ui/react-accordion");
    ...
    expect(overview).not.toContain("OpenClaw ·");
    ...
    expect(overview).toContain("data.providerSummary.connected");
    expect(overview).toContain("data.providerSummary.total");
    expect(overview).not.toContain('className="stat"');
    expect(overview).not.toContain("stat-value");
    expect(overview).not.toContain("Counts only model providers from the live gateway catalog");
    expect(overview).not.toContain("ModelProvidersPanel");
    expect(overview).not.toContain("Connect provider");
    expect(overview).toContain("data-health-status={summary.status}");
    expect(overview).toContain("data-health-attention-count={summary.attention}");
    expect(overview).toContain("data-health-checked-at={health.checkedAt");
    expect(overview).toContain("Health warning:");
    expect(overview).toContain("HealthCheckSubmitButton");
    expect(healthCheckButton).toContain("Run health check");
    ...
    expect(autoRefresh).toContain("window.clearInterval(interval)");
    expect(autoRefresh).toContain("refreshScheduled.current");
