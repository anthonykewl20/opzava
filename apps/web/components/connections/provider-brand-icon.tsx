...
type ProviderBrandSource = "simple-icons-16.26.0" | "mainframe-docs-sponsor" | "monogram";

interface SvgBrand {
  readonly kind: "brand";
  readonly color: string;
  readonly path: string;
  readonly source: Exclude<ProviderBrandSource, "monogram">;
}

interface MonogramBrand {
...
/*
 * Brand provenance:
 * - 13 unchanged SVG path geometries come from Simple Icons 16.26.0 (CC0).
 * - OpenAI is not a Simple Icons asset. Its unchanged geometry is vendored in
 *   mainframe/docs/assets/sponsors/openai.svg. The trademark is retained for
 *   nominative identification; no redistribution-license claim is made here.
...
const providerBrands = {
  ...
  groq: { kind: "monogram", color: "var(--info)", source: "monogram", monogram: "G" },
  xai: { kind: "monogram", color: "var(--success)", source: "monogram", monogram: "X" },
  cerebras: {
    kind: "monogram",
  ...
  together: { kind: "monogram", color: "var(--info)", source: "monogram", monogram: "T" },
  fireworks: {
    kind: "monogram",
    color: "var(--warning)",
    source: "monogram",
...
export function ProviderBrandIcon({ providerId, label, className }: ProviderBrandIconProps) {
  ...
  const style = {
    ...
    backgroundColor: "color-mix(in srgb, var(--provider-brand) 12%, var(--muted))",
  } as CSSProperties;
  const sharedProps = {
    "aria-hidden": true,
  ...
  return (
    ...
    >
      <span className="text-xs font-semibold leading-none">{monogram}</span>
    </span>
  );
}
